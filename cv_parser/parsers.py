...
class AdvancedDocumentParser:
    def __init__(self):
        ...
        try:
            ...
            self.logger.warning("pytesseract not installed, OCR functionality will be limited")
        ...
        try:
            ...
            self.logger.warning("pdf2image not installed, using alternative PDF extraction method")
        ...
        try:
            ...
            self.logger.warning("docx2python not installed, using alternative DOCX extraction method")
        ...
        if not self.deepseek_api_key:
            self.logger.warning("DeepSeek API key not found, CV segmentation will not be available")
    ...
    def parse_pdf(self, file_path: str) -> str:
        ...
        try:
                    ...
                    with open(file_path, 'rb') as file:
                        ...
                        if pages_text:
                            ...
                            if pypdf2_text.strip():
                                ...
                                elif len(pypdf2_text.split()) > len(extracted_text.split()) * 1.5:
                                    extracted_text = pypdf2_text
                                    extraction_methods.append("PyPDF2")
                except Exception as e:
            ...
            if extraction_methods:
                logger.info(f"PDF text extracted using: {', '.join(extraction_methods)}")
            ...
            if extracted_text:
                ...
                extracted_text = re.sub(r'([a-zA-Z0-9_.+-]+)\s*@\s*([a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+)', r'\1@\2', extracted_text)
    ...
    def _ocr_pdf(self, file_path: str) -> str:
        ...
        if not PYTESSERACT_AVAILABLE:
            logger.warning("OCR requirements (pytesseract/PIL) not available. Skipping OCR.")
            return ""
            
        if not PDFPLUMBER_AVAILABLE:
            logger.warning("pdfplumber not available for PDF to image conversion. Skipping OCR.")
            return ""
        ...
        try:
            ...
            if combined_text:
                ...
                combined_text = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\xff]', '', combined_text)  # Remove non-printable chars
                combined_text = re.sub(r'(?<!\w)I(?!\w)', '1', combined_text)  # Fix common I -> 1 confusion
                combined_text = re.sub(r'(?<!\w)O(?!\w)', '0', combined_text)  # Fix common O -> 0 confusion
                combined_text = re.sub(r'[\r\n]+', '\n', combined_text)  # Normalize line breaks
    ...
    def parse_docx(self, file_path: str) -> str:
        ...
        try:
            ...
            logger.info(f"Standard docx parsing returned minimal text, trying XML-based parsing for {file_path}")
    ...
    def extract_personal_info(self, text: str) -> Dict[str, Any]:
        ...
        if not result["email"]:
            ...
            for email_match in email_matches:
                ...
                if (len(potential_email) < 50 and 
                    '/' not in potential_email and
                    not re.search(r'example|test|sample|demo', potential_email, re.IGNORECASE)):
        ...
        if not result["first_name"] and not result["last_name"]:
            ...
            for pattern in name_patterns:
                ...
                if name_match:
                    ...
                    contains_title = any(re.search(r'\b' + re.escape(prefix) + r'\b', potential_name, re.IGNORECASE) for prefix in title_prefixes)
                    contains_section = any(re.search(r'\b' + re.escape(section) + r'\b', potential_name, re.IGNORECASE) for section in section_keywords)
                    
                    if not contains_title and not contains_section and len(potential_name.split()) <= 3:
            ...
            if not result["first_name"]:
                ...
                for line in first_lines:
                    ...
                    if (not line or 
                        any(section in line.lower() for section in section_keywords) or
                        len(line) > 40):
                        continue
                    ...
                    if 1 <= len(words) <= 3 and all(word[0].isupper() for word in words if word):
                        ...
                        contains_title = any(re.search(r'\b' + re.escape(prefix) + r'\b', line, re.IGNORECASE) for prefix in title_prefixes)
        ...
        if not result["location"]:
            ...
            for pattern in location_patterns:
                location_match = re.search(pattern, text[:2000])  # Look in first 2000 chars
                if location_match:
                    ...
                    if (len(potential_location) < 50 and 
                        not any(title in potential_location for title in title_prefixes) and
                        not any(section in potential_location.lower() for section in section_keywords)):
    ...
    def _extract_education(self, text: str) -> List[Dict[str, Any]]:
        ...
        if 'education' in sections:
            ...
            logger.debug(f"Found dedicated education section ({len(education_text)} chars)")
        else:
            ...
            education_section_match = re.search(education_section_pattern, text, re.IGNORECASE | re.DOTALL)
            
            if education_section_match:
                education_text = education_section_match.group(1)
                logger.debug(f"Found education section via pattern matching ({len(education_text)} chars)")
            else:
        ...
        for block in combined_blocks:
            ...
            for pattern in uk_education_patterns:
                ...
                if match:
                    ...
                    # Extract fields
                    institution = match.group('institution').strip() if 'institution' in match.groupdict() and match.group('institution') else ""
                    degree = match.group('degree').strip() if 'degree' in match.groupdict() and match.group('degree') else ""
                    major = match.group('major').strip() if 'major' in match.groupdict() and match.group('major') else ""
                    ...
                    if 'A level' in degree.lower() and not major:
                        ...
                        subjects_match = re.search(r'A\s*[Ll]evels?[:\s]+in\s+([^,\.]+)', block)
                        if subjects_match:
                            major = subjects_match.group(1).strip()
                        else:
                            # Try to find common A level subjects
                            for subject in ["Mathematics", "English", "Physics", "Chemistry", "Biology", 
                    ...
                    logger.debug(f"Found UK education format: {degree} at {institution}")
                    break
            ...
            if not uk_match_found:
                ...
                if degree_match:
                    # Extract fields
                    institution = degree_match.group('institution').strip() if degree_match.group('institution') else ""
                    degree = degree_match.group('degree').strip()
                    major = degree_match.group('major').strip() if degree_match.group('major') else ""
                    
                    # Extract year if present
                    year = ""
                    if 'year' in degree_match.groupdict() and degree_match.group('year'):
                    ...
                    institution = re.sub(r'[,.]$', '', institution)  # Remove trailing commas or periods
                    ...
                    logger.debug(f"Found standard education format: {degree} at {institution}")
                    ...
                    university_match = re.search(university_pattern, block, re.IGNORECASE)
                    
                    if university_match:
                        # Extract fields
                        institution = university_match.group('institution').strip()
                        degree = university_match.group('degree').strip() if university_match.group('degree') else ""
                        major = university_match.group('major').strip() if university_match.group('major') else ""
                        
                        # Extract year if present
                        year = ""
                        if 'year' in university_match.groupdict() and university_match.group('year'):
        ...
        if not education_entries:
            ...
            uk_keywords = {
                "A level": ["A levels", "A Levels", "A-levels", "A-Levels", "A level", "A Level"],
            ...
            for degree_type, keywords in uk_keywords.items():
                for keyword in keywords:
                    if keyword in education_text:
                        ...
                        search_window = education_text[max(0, keyword_pos - 100):min(len(education_text), keyword_pos + 100)]
                        
                        institution_match = re.search(r'([A-Z][a-z]+\s+(?:' + '|'.join(uk_institutions) + r'))', search_window)
                        institution = institution_match.group(1) if institution_match else ""
                        
                        if not institution:
                            # Try more generic institution pattern
                            generic_institution_match = re.search(r'([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3})', search_window)
                            if generic_institution_match:
                                potential_institution = generic_institution_match.group(1)
                                # Only use if it seems like a valid institution name
                                if len(potential_institution) > 5 and not any(term in potential_institution.lower() 
                                                                             for term in ["date", "present", "current", "year"]):
                                    institution = potential_institution
                        
                        # Look for major/subject
                        major = ""
                        major_match = re.search(keyword + r'[:\s]+in\s+([^,\.]+)', search_window, re.IGNORECASE)
                        if major_match:
                        ...
                        if institution or major:
                            ...
                            logger.debug(f"Found keyword-based UK education: {degree_type} at {institution}")
            ...
            if "Norwich University" in education_text or "Norwich" in education_text:
                ...
                degree_match = re.search(r'(BBA|BA|BSc|B\.B\.A\.|B\.A\.|B\.Sc\.)\s*(?:\([Hh]ons?\))?', education_text)
                if degree_match:
                    degree = degree_match.group(0)
                
                # Look for major
                major = ""
                major_match = re.search(r'(?:BBA|BA|BSc|B\.B\.A\.|B\.A\.|B\.Sc\.)\s*(?:\([Hh]ons?\))?\s+in\s+([^,\.]+)', education_text)
                if major_match:
                    major = major_match.group(1).strip()
            ...
            if "Ipswich Town College" in education_text or "Ipswich College" in education_text:
                # Look for A levels
                degree = "A levels"
                ...
                common_subjects = ["Mathematics", "English", "Physics", "Chemistry", "Biology", 
                                 "History", "Geography", "Economics", "Business Studies"]
        ...
        if 'summary' in sections:
            ...
            for pattern in summary_section_patterns:
                ...
                if summary_match:
                    ...
                    logger.debug(f"Found summary section via pattern matching ({len(summary_text)} chars)")
                    return self._format_summary(summary_text)
            ...
            for i, paragraph in enumerate(paragraphs[:5]):  # Only check first 5 paragraphs
                ...
                if re.search(r'\b(?:EXPERIENCE|EDUCATION|SKILLS|CERTIFICATIONS|LANGUAGES)\b', paragraph.upper()):
                    continue
                
                # Skip if it's likely personal info
                if re.search(r'\b(?:Email|Phone|Address|Location|LinkedIn)\b', paragraph, re.IGNORECASE):
                    continue
                ...
                for indicator in summary_indicators:
                    if re.search(indicator, paragraph, re.IGNORECASE):
                        logger.debug(f"Extracted professional summary from document beginning ({len(paragraph)} chars)")
                ...
                if len(paragraph) > 100 and len(paragraph) < 1000 and i > 0:
                    logger.debug(f"Extracted potential professional summary from document beginning ({len(paragraph)} chars)")
                    return self._format_summary(paragraph)
    ...
    def _format_summary(self, summary: str) -> str:
        ...
        summary = re.sub(r'^(?:SUMMARY|PROFESSIONAL\s+SUMMARY|PROFILE|PERSONAL\s+SUMMARY)[:.\s-]*', '', summary, flags=re.IGNORECASE)
        ...
        if len(summary) > max_summary_length:
            logger.debug(f"Truncating overly long summary from {len(summary)} to {max_summary_length} chars")
            # Try to truncate at a sentence boundary
            truncated = summary[:max_summary_length]
            last_period = truncated.rfind('.')
            if last_period > max_summary_length * 0.7:  # Only truncate at period if it's not too short
        ...
        if summary and not summary.endswith('.') and not summary.endswith('!') and not summary.endswith('?'):
            ...
            if last_period > 0 and last_period > len(summary) * 0.5:  # Only if we're not losing too much
    ...
    def parse_document(self, file_path, text=None, max_timeout=180):
        ...
        if len(text) > 15000:
            ...
            self.logger.info(f"Optimized large text from {len(text)} to {len(optimized_text)} characters")
            text = optimized_text
        ...
        def should_abort():
            ...
            if elapsed > max_timeout:
                self.logger.warning(f"Parser operation approaching timeout after {elapsed:.2f} seconds")
                return True
            return False
        ...
        if segmented_text:
            try:
                ...
                if meaningful_content:
                    ...
                    elapsed_time = time.time() - start_time
                    self.logger.info(f"Total parsing time with DeepSeek segmentation: {elapsed_time:.2f} seconds")
                    return result
                else:
                    self.logger.warning("Segmented text parsing produced empty or invalid results")
            except Exception as e:
        ...
        if self._should_use_llm_parsing() and not should_abort():
            try:
                ...
                llm_timeout = min(30, max_timeout * 0.2)  # Use at most 20% of remaining time
                llm_end_time = time.time() + llm_timeout
                result = self._parse_with_llm(text, timeout=llm_timeout)
                elapsed_time = time.time() - start_time
                self.logger.info(f"Total parsing time with LLM: {elapsed_time:.2f} seconds")
                return result
            except Exception as e:
    ...
    def _parse_with_llm(self, text, timeout=30):
        ...
        try:
            ...
            try:
                ...
                for key in ["personal_info", "professional_summary", "education", "experience", "skills", "languages", "certifications"]:
                    if key not in result:
                        result[key] = empty_result[key]
                        
                self.logger.info(f"Successfully parsed CV with LLM in {time.time() - start_time:.2f} seconds")
                return result
    ...
    def _parse_traditional(self, text, timeout=90):
        ...
        if len(text) > 50000:
            self.logger.warning(f"CV text is very large ({len(text)} chars), truncating to first 50000 chars")
            text = text[:50000]
        ...
        def should_abort():
            ...
            if elapsed > timeout * 0.9:  # 90% of timeout
                self.logger.error(f"Parser operation timed out after {elapsed:.2f} seconds")
                return True
            return False
        ...
        for industry, check_method in industry_checks.items():
            if time.time() - industry_check_start > industry_check_timeout:
                self.logger.warning("Industry check timeout exceeded, using general CV type")
                break
        ...
        try:
            ...
            self.logger.info(f"Identified {len(sections)} sections in CV (took {time.time() - section_start_time:.2f}s)")
        except TimeoutError:
            self.logger.warning("Section identification timed out, using empty sections")
            sections = {}
        ...
        try:
            ...
            if time.time() - start_time < personal_info_timeout:
                ...
                self.logger.info(f"Extracted personal info with {len(cv_data['personal_info'])} fields (took {time.time() - personal_info_start:.2f}s)")
            else:
            ...
            if time.time() - start_time < summary_timeout:
                if "summary" in sections:
                    ...
                    cv_data["professional_summary"] = self.extract_professional_summary(text)
                self.logger.info(f"Extracted professional summary (took {time.time() - summary_start:.2f}s)")
            else:
                self.logger.warning("Skipping professional summary extraction due to timeout")
            ...
            if time.time() - start_time < education_timeout:
                cv_data["education"] = self._extract_education_efficient(text, sections, education_timeout)
                self.logger.info(f"Extracted {len(cv_data['education'])} education entries (took {time.time() - education_start:.2f}s)")
            else:
            ...
            if time.time() - start_time < experience_timeout:
                cv_data["experience"] = self._extract_experience_efficient(text, sections)
                self.logger.info(f"Extracted {len(cv_data['experience'])} experience entries (took {time.time() - experience_start:.2f}s)")
            else:
            ...
            if time.time() - start_time < skills_timeout:
                ...
                self.logger.info(f"Extracted {len(cv_data['skills'])} skills (took {time.time() - skills_start:.2f}s)")
            else:
            ...
            if time.time() - start_time < languages_timeout:
                if "languages" in sections:
                    cv_data["languages"] = self._parse_languages_section(sections["languages"])
                else:
                    cv_data["languages"] = self._extract_languages(text)
                self.logger.info(f"Extracted {len(cv_data['languages'])} languages (took {time.time() - languages_start:.2f}s)")
            else:
            ...
            if time.time() - start_time < certifications_timeout:
                if "certifications" in sections:
                    cv_data["certifications"] = self._parse_certifications_section(sections["certifications"])
                else:
                    cv_data["certifications"] = self._extract_certifications(text)
                self.logger.info(f"Extracted {len(cv_data['certifications'])} certifications (took {time.time() - certifications_start:.2f}s)")
            else:
            ...
            if not should_abort():
                ...
                if time.time() - start_time < projects_timeout:
                    if "projects" in sections:
                        cv_data["projects"] = self._parse_projects_section(sections["projects"])
                    elif hasattr(self, '_extract_projects'):  # Check if method exists
                        cv_data["projects"] = self._extract_projects(text)
                    self.logger.info(f"Extracted {len(cv_data['projects'])} projects (took {time.time() - projects_start:.2f}s)")
                else:
                ...
                if time.time() - start_time < interests_timeout:
                    if "interests" in sections:
                        cv_data["interests"] = self._parse_interests_section(sections["interests"])
                    elif hasattr(self, '_extract_interests'):  # Check if method exists
                        cv_data["interests"] = self._extract_interests(text)
                    self.logger.info(f"Extracted {len(cv_data['interests'])} interests (took {time.time() - interests_start:.2f}s)")
        except Exception as e:
        ...
        # Log processing time
        end_time = time.time()
        self.logger.info(f"Traditional parsing completed in {end_time - start_time:.2f} seconds")
    ...
    def _extract_education_efficient(self, text, sections=None, timeout=10.0):
        ...
        if sections and 'education' in sections:
            start_pos, end_pos = sections['education']
            education_text = text[start_pos:end_pos]
            self.logger.debug(f"Using pre-identified education section ({len(education_text)} chars)")
        else:
            ...
            for pattern in education_section_patterns:
                ...
                if education_match:
                    ...
                    self.logger.debug(f"Found education section via pattern matching ({len(education_text)} chars)")
                    break
            
            if not education_text:
                # Use entire text but be more strict about what we extract
                education_text = text
                self.logger.debug("No dedicated education section found, using full text with stricter extraction")
        ...
        fields_of_study = [
            "Computer Science", "Engineering", "Business", "Management", "Finance", 
            ...
            "Communications", "Marketing", "Accounting", "Law", "Medicine", 
            "Nursing", "Education", "Art", "Design", "Architecture", "Music"
        ]
        ...
        for pattern in degree_patterns:
            ...
            for match in matches:
                ...
                if "from|at" in pattern:  # Degree first then institution
                    ...
                    degree_match = re.search(r'(Bachelor|Master|MBA|PhD|Doctorate|Associate|B\.?S\.?|B\.?A\.?|M\.?S\.?|M\.?A\.?|M\.?B\.?A\.?|Ph\.?D\.?|J\.?D\.?|M\.?D\.?)', 
                ...
                surrounding_text = education_text[max(0, match.start() - 50):min(len(education_text), match.end() + 50)]
                date_match = re.search(r'(\d{4})\s*(?:-|–|to)\s*(\d{4}|Present|Current)', surrounding_text, re.IGNORECASE)
                if date_match:
        ...
        if not education_entries and not should_abort():
            ...
            for paragraph in paragraphs[:10]:  # Limit to first 10 paragraphs to avoid processing too much
                if should_abort():
                    break
                ...
                if degree_match:
                    ...
                    field_match = re.search(r'\b(?:of|in)\s+([A-Z][A-Za-z\s]+)', paragraph[degree_match.end():], re.IGNORECASE)
                    if field_match:
                    ...
                    for degree_abbr, full_degree in degree_mappings.items():
                        if re.search(r'\b' + re.escape(degree_abbr) + r'\b', paragraph, re.IGNORECASE):
                ...
                for pattern in institution_patterns:
                    ...
                    if institution_match:
                        if 'of' in pattern and len(institution_match.groups()) > 1:
                            entry["institution"] = f"{institution_match.group(1)} of {institution_match.group(2)}"
                        else:
                ...
                if not entry["major"]:
                    for field in fields_of_study:
                        if re.search(r'\b' + re.escape(field) + r'\b', paragraph, re.IGNORECASE):
                            entry["major"] = field
                            break
                
                # Try to extract dates
                date_matches = [
                    re.search(r'(\d{4})\s*(?:-|–|to)\s*(\d{4}|Present|Current)', paragraph),
                ...
                if len(education_entries) >= 5:  # Usually CVs don't have more than 5 education entries
                    break
        ...
        if not education_entries and not should_abort():
            ...
            for degree_abbr, full_degree in degree_mappings.items():
                ...
                matches = re.finditer(r'\b' + re.escape(degree_abbr) + r'\b', education_text, re.IGNORECASE)
                for match in matches:
                    entry = {
                        "institution": "",
                        "degree": full_degree,
                    ...
                    surrounding_text = education_text[max(0, match.start() - 100):min(len(education_text), match.end() + 100)]
                    institution_match = re.search(r'\b([A-Z][A-Za-z\s]+(?:University|College|School|Institute|Academy))\b', surrounding_text)
                    if institution_match:
                        entry["institution"] = institution_match.group(1).strip()
                    
                    # Look for dates near the degree
                    date_match = re.search(r'(\d{4})\s*(?:-|–|to)\s*(\d{4}|Present|Current)', surrounding_text)
                    if date_match:
                        entry["dates"] = f"{date_match.group(1)}-{date_match.group(2)}"
                    else:
                        year_match = re.search(r'\b(19|20)\d{2}\b', surrounding_text)
        ...
        for category, patterns in tech_skill_patterns.items():
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if skill.upper() in ["HTML", "CSS", "PHP", "SQL", "AWS", "API", "REST", "JSON", "XML", "UI", "UX", 
                                        "CI", "CD", "QA", "AI", "ML", "AR", "VR", "iOS", "AWS", "GCP"]:
                        skill = skill.upper()
    ...
    def _extract_experience(self, text: str) -> List[Dict[str, Any]]:
        ...
        if 'experience' in sections:
            ...
            if experience_section_match:
                ...
                logger.debug(f"Found experience section via pattern matching ({len(experience_text)} chars)")
            else:
                # Fallback to using full text
        ...
        for block in experience_blocks:
            ...
            if not title and dates:
                ...
                if date_pos > 0:
                    ...
                    title_candidates = re.findall(r'([A-Z][A-Za-z\s]+)', text_before_date)
                    if title_candidates:
                        title = title_candidates[-1].strip()  # Use the last one as it's likely closest to the date
                    ...
                    title_candidates = re.findall(r'([A-Z][A-Za-z\s]+)', text_after_date)
                    if title_candidates:
                        title = title_candidates[0].strip()  # Use the first one after the date
            ...
            if title:
                ...
                if re.search(r'\d{4}|Inc\.?|LLC|Ltd\.?|Limited|Corporation|Corp\.?', title):
                    title = ""
            ...
            start_markers = []
            if title:
                start_markers.append((block.find(title) + len(title), f"after title '{title}'"))
            if company:
                start_markers.append((block.find(company) + len(company), f"after company '{company}'"))
            if dates:
                start_markers.append((block.find(dates) + len(dates), f"after dates '{dates}'"))
            
            if start_markers:
                # Use the latest start position
                start_pos, marker_info = max(start_markers, key=lambda x: x[0])
                logger.debug(f"Starting description {marker_info} at position {start_pos}")
            ...
            if description:
                ...
                if len(description) > 1000:
                    logger.debug(f"Truncating overly long description from {len(description)} chars")
    ...
    def _extract_skills(self, text: str) -> List[str]:
        ...
        if 'skills' in sections:
            ...
            for pattern in skills_section_patterns:
                ...
                if skills_match:
                    ...
                    logger.debug(f"Found skills section via pattern matching ({len(skills_text)} chars)")
                    break
            ...
            if not skills_text:
                ...
                if 'experience' in sections:
                    ...
                    start_pos, end_pos = sections['experience']
                    experience_text = text[start_pos:end_pos]
                    skills_from_exp = self._extract_skills_from_experience(experience_text)
        ...
        technical_skills = [
            ...
            "Kotlin", "Go", "Rust", "Scala", "Perl", "Shell", "Bash", "SQL", "R", "MATLAB",
            
            # Web technologies
            "HTML", "CSS", "React", "Angular", "Vue", "Node.js", "Express", "Django", "Flask",
            "ASP.NET", "Spring", "Ruby on Rails", "jQuery", "Bootstrap", "Tailwind",
            
            # Data & Analytics
            "Data Analysis", "Machine Learning", "Deep Learning", "AI", "NLP", "Computer Vision",
            "Data Mining", "Big Data", "Hadoop", "Spark", "Tableau", "Power BI", "Data Visualization",
            "Statistics", "Data Science", "Data Engineering", "ETL", "Data Warehousing",
            
            # Cloud & DevOps
            "AWS", "Azure", "GCP", "Docker", "Kubernetes", "Jenkins", "CI/CD", "Git", "GitHub",
            "GitLab", "Terraform", "Ansible", "Puppet", "Chef", "Prometheus", "Grafana", "ELK Stack",
            
            # Mobile
            "Android", "iOS", "React Native", "Flutter", "Swift", "Kotlin", "Objective-C",
            
            # Other tech
            "Agile", "Scrum", "Kanban", "JIRA", "REST API", "GraphQL", "Microservices", "SOA", "Big Data", "Hadoop", "Spark",
            
            # Design
            "UX/UI", "Figma", "Sketch", "Adobe XD", "Photoshop", "Illustrator", "InDesign",
            "Graphic Design", "Web Design", "Product Design", "User Research", "Wireframing",
        ...
        soft_skills = [
            ...
            "Customer Service", "Networking", "Mentoring", "Coaching", "Strategic Planning",
            "Attention to Detail", "Organization", "Multitasking", "Research", "Analytical Thinking"
        ]
        
        industry_skills = {
            "Marketing": ["Digital Marketing", "SEO", "SEM", "Content Marketing", "Social Media Marketing",
                         "Email Marketing", "Marketing Automation", "Google Analytics", "CRM", "Branding",
                         "Market Research", "Campaign Management", "Google Ads", "Facebook Ads", "Copywriting",
                         "Sales", "Business Development", "Account Management", "Lead Generation"],
            
            "Finance": ["Financial Analysis", "Financial Modeling", "Accounting", "Budgeting", "Forecasting",
                      "Valuation", "Risk Management", "Investment Banking", "Portfolio Management",
                      "Financial Reporting", "QuickBooks", "SAP", "Bloomberg Terminal", "Excel Modeling"],
            
            "Healthcare": ["Patient Care", "Electronic Health Records", "HIPAA", "Clinical Documentation",
                         "Medical Coding", "Medical Billing", "Healthcare Administration", "Epic", "Cerner"],
            
            "Media": ["Video Production", "Video Editing", "Motion Graphics", "Animation", "Broadcasting",
                    "Content Creation", "Final Cut Pro", "Adobe Premiere", "After Effects", "Avid",
                    "Cinematography", "Directing", "Scriptwriting", "Storyboarding", "IPTV", "OTT", "DVB"]
        }
        ...
        def is_valid_skill(skill):
            ...
            unwanted_patterns = [
                r'@', r'://', r'\\', r'\d{3,}',  # emails, URLs, paths, long numbers
            ...
            job_titles = [
                "Chief System Architect", "SVP System Integration", "Chief Technology Officer", 
                "System Architect", "Software Engineer", "Project Manager", "Product Manager",
                "Director", "Vice President", "President", "CEO", "CTO", "COO", "CFO", 
            ...
            if any(re.search(rf'\b{re.escape(s)}\b', clean_skill, re.IGNORECASE) for s in technical_skills):
                return True
            
            if any(re.search(rf'\b{re.escape(s)}\b', clean_skill, re.IGNORECASE) for s in soft_skills):
                return True
            
            for industry, industry_skill_list in industry_skills.items():
                if any(re.search(rf'\b{re.escape(s)}\b', clean_skill, re.IGNORECASE) for s in industry_skill_list):
                    return True
            ...
            media_skills = [
                "Broadcasting", "Video Production", "Video Editing", "Post Production",
                "Content Delivery", "Digital Media", "Media Server", "Cable TV",
                "Mobile TV", "Streaming", "Cloud Video", "Video Compression",
                "Encoding", "Transcoding", "Video Distribution", "Video Storage",
                "Media Asset Management", "Video Analytics", "Digital Rights Management",
                "Content Management", "Video Workflow", "Media Workflow", "Set-top Box",
                "Android TV", "Apple TV", "Smart TV", "Broadcast Automation",
                "Live Production", "Remote Production", "Virtual Production"
            ]
            ...
            for pattern in skill_patterns:
                if re.match(pattern, clean_skill):
                    ...
                    common_words = ["about", "above", "across", "after", "again", "against", "almost", "alone", 
                                   "along", "already", "also", "although", "always", "among", "another", 
                                   "before", "behind", "being", "below", "between", "both", "company", "city", 
                                   "state", "each", "either", "enough", "every", "everybody", "everyone", 
                                   "everything", "everywhere", "except", "executive", "finally", "first", 
                                   "following", "further", "furthermore", "general", "however", "indeed", 
                                   "instead", "itself", "looking", "mainly", "maybe", "meanwhile", "moreover", 
                                   "mostly", "namely", "neither", "nevertheless", "next", "nobody", "nothing", 
                                   "nowhere", "often", "otherwise", "overall", "particularly", "perhaps", 
                                   "possibly", "generally", "rather", "regarding", "second", "secondly", 
                                   "similarly", "since", "slightly", "someone", "something", "sometimes", 
                                   "somewhat", "somewhere", "specifically", "still", "strongly", "there", 
                                   "thereafter", "thereby", "therefore", "though", "through", "throughout", 
                                   "thus", "together", "toward", "towards", "under", "underneath", "undoubtedly", 
                                   "unless", "unlike", "until", "upon", "usually", "when", "where", "whereas", 
                                   "wherever", "whether", "which", "while", "within", "without", "would", 
                                   "responsibilities", "accomplishments", "experience"]
            ...
            tech_companies_as_skills = ["Cisco", "Yahoo", "Microsoft", "Google", "Amazon", "AWS", "Oracle", "IBM", "SAP"]
        ...
        skill_highlights_match = re.search(skill_highlights_pattern, text, re.IGNORECASE | re.DOTALL)
        
        if skill_highlights_match:
            highlight_text = skill_highlights_match.group(1)
            # Split by newlines or bullet points
            highlight_skills = re.findall(r'[•\-–—*★⭐✓✔+]?\s*([^•\-–—*★⭐✓✔+\n,]{3,50})(?:,|\n|$)', highlight_text)
        ...
        if len(skills) < 5:
            ...
            experience_skills_matches = re.finditer(experience_skills_pattern, text, re.IGNORECASE)
        ...
        for skill in skills:
            ...
            if not is_duplicate and len(unique_skills) < 30:  # Limit to 30 skills maximum
    ...
    def _extract_languages(self, text: str) -> List[Dict[str, str]]:
        ...
        if 'languages' in sections:
            ...
            logger.debug(f"Found dedicated languages section ({len(languages_text)} chars)")
        else:
            # Try traditional section identification as fallback
            ...
            languages_section_match = re.search(languages_section_pattern, text, re.IGNORECASE | re.DOTALL)
            
            if languages_section_match:
                languages_text = languages_section_match.group(1)
                logger.debug(f"Found languages section via pattern matching ({len(languages_text)} chars)")
            else:
        ...
        common_languages = [
            ...
            "Bengali", "Urdu", "Turkish", "Polish", "Swedish", "Norwegian", "Danish", 
            "Finnish", "Greek", "Hebrew", "Thai", "Vietnamese", "Malay", "Indonesian"
        ]
        
        # Common proficiency levels
        proficiency_levels = [
            "Native", "Fluent", "Proficient", "Advanced", "Intermediate", "Basic", "Beginner",
            "Business", "Conversational", "Elementary", "Professional", "C2", "C1", "B2", "B1", "A2", "A1"
        ]
        ...
        for language in common_languages:
            ...
            if language not in language_with_level:
                ...
                if re.search(r'\b' + re.escape(language) + r'\b', languages_text, re.IGNORECASE):
    ...
    def _extract_certifications(self, text: str) -> List[Dict[str, str]]:
        ...
        if 'certifications' in sections:
            ...
            logger.debug(f"Found dedicated certifications section ({len(certifications_text)} chars)")
        else:
            ...
            for pattern in certification_section_patterns:
                ...
                if cert_section_match:
                    ...
                    logger.debug(f"Found certifications section via pattern matching ({len(certifications_text)} chars)")
                    break
            ...
            if not certifications_text:
                ...
                certifications_text = text[:min(len(text), 10000)]  # Limit to first 10000 chars
                logger.debug("No dedicated certifications section found, using limited text")
        ...
        for pattern in specific_certifications:
            ...
            for match in matches:
                ...
                for existing_cert in valid_certs:
                    if cert_text.lower() in existing_cert['name'].lower() or existing_cert['name'].lower() in cert_text.lower():
                        is_unique = False
                        break
        ...
        if len(valid_certs) < 5:  # Only if we haven't found enough specific certs
            ...
            exclude_words = ["EXPERIENCE", "PROJECTS", "LANGUAGES", "EDUCATION", "SKILLS", 
                             "ADDITIONAL", "CONTACT", "PERSONAL", "SUMMARY", "REFERENCE", 
                             "OBJECTIVE", "PROFILE", "WORK", "EMPLOYMENT"]
            ...
            for pattern in bullet_patterns:
                ...
                for match in bullet_matches:
                    ...
                    for word in exclude_words:
                        if re.search(r'\b' + re.escape(word) + r'\b', cert_text, re.IGNORECASE):
                            should_skip = True
                            break
                    ...
                    cert_keywords = ["Certified", "Certificate", "Certification", "License", "Licensed", 
                                    "Diploma", "Degree", "Bachelor", "Master", "MBA", "PhD", "Doctorate"]
                    
                    if not is_valid_cert:
                        for keyword in cert_keywords:
                            if re.search(r'\b' + re.escape(keyword) + r'\b', cert_text, re.IGNORECASE):
                                is_valid_cert = True
                                break
                    ...
                    cert_name = re.sub(r'[.,:;]$', '', cert_text)  # Remove trailing punctuation
                    
                    # Check if this certification is unique
                    is_unique = True
                    for existing_cert in valid_certs:
                        if cert_name.lower() in existing_cert['name'].lower() or existing_cert['name'].lower() in cert_name.lower():
                            is_unique = False
                            break
            ...
            if len(valid_certs) < 5:
                ...
                for pattern in cert_line_patterns:
                    line_matches = re.finditer(pattern, certifications_text, re.MULTILINE)
                    for match in matches:
    ...
    def _parse_segmented_text(self, segmented_text):
        ...
        full_name = ""
        first_line_match = re.match(r'^([A-Z][a-zA-Z\s\.-]+)(?:\n|,|\s{2,})', cleaned_text, re.MULTILINE)
        if first_line_match:
        ...
        phone_match = re.search(r'(?:\+\d{1,2}\s?)?(?:\(\d{3}\)|\d{3})[-.\s]?\d{3}[-.\s]?\d{4}', cleaned_text)
        if phone_match:
            result["personal_info"]["phone"] = phone_match.group(0)
            
        location_match = re.search(r'(?:City\s*,\s*State|[A-Z][a-zA-Z\s]+,\s*[A-Z]{2}|[A-Z][a-zA-Z\s]+,\s*[A-Z][a-zA-Z\s]+)', cleaned_text)
        if location_match:
        ...
        # Look for LinkedIn
        linkedin_match = re.search(r'(?:linkedin\.com/in/|LinkedIn:)\s*([a-zA-Z0-9/-]+)', cleaned_text)
        if linkedin_match:
        ...
        for i in range(0, len(sections), 2):
            ...
            if (section_content.lower() == "(content)" or 
                section_content.lower() == "content" or
                section_content.lower() == "(not provided)" or
                section_content.lower() == "(no content)"): 
            ...
            section_keywords = ['PERSONAL', 'SUMMARY', 'EXPERIENCE', 'EDUCATION', 'SKILLS', 
                               'LANGUAGES', 'CERTIFICATIONS', 'PROJECTS', 'INTERESTS']
            
            # If content has section markers, extract only the relevant part
            if "==========" in section_content:
                logger.warning(f"Found section markers within section content - cleaning up")
            ...
            if any(section_content.strip().upper().startswith(kw) for kw in section_keywords):
                # Try to find where the actual content starts
                lines = section_content.split('\n', 2)
                if len(lines) > 1 and any(kw in lines[0].upper() for kw in section_keywords):
                    logger.warning(f"Section content starts with section name - cleaning up")
            ...
            try:
                ...
                if "personal" in section_name:
                    ...
                    for key, value in personal_info.items():
                        if value and any(kw in value.upper() for kw in section_keywords):
                            personal_info[key] = ""
                    ...
                    if not any(personal_info.values()):
                        ...
                        name_match = re.search(r'^([A-Z][a-zA-Z\s\.-]+)(?:\n|,|\s{2,})', section_content[:500], re.MULTILINE)
                        if name_match:
                        ...
                        # Try to find email
                        email_match = re.search(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', section_content)
                        if email_match:
                            personal_info["email"] = email_match.group(0)
                        
                        # Try to find phone
                        phone_match = re.search(r'(?:\+\d{1,2}\s?)?(?:\(\d{3}\)|\d{3})[-.\s]?\d{3}[-.\s]?\d{4}', section_content)
                        if phone_match:
                            personal_info["phone"] = phone_match.group(0)
                        
                        # Try to find location
                        location_match = re.search(r'(?:City\s*,\s*State|[A-Z][a-zA-Z\s]+,\s*[A-Z]{2}|[A-Z][a-zA-Z\s]+,\s*[A-Z][a-zA-Z\s]+)', section_content)
                        if location_match:
                            personal_info["location"] = location_match.group(0).replace("City , State", "").strip()
                ...
                # Process Professional Summary
                    ...
                    if "==========" in summary or any(f"========{kw}" in summary.upper() for kw in section_keywords):
                        # Try to clean it
                        summary = re.sub(r'={5,}[A-Z_]+.*$', '', summary, flags=re.DOTALL).strip()
                    ...
                    if "==========" in summary:
                        ...
                        valid_parts = [p for p in parts if len(p) > 30 and not any(kw in p.upper() for kw in section_keywords)]
                        if valid_parts:
                ...
                # Process Education
                    ...
                    valid_entries = []
                    for entry in education_entries:
                        if not any(marker in str(entry.values()) for marker in ["==========", "PERSONAL_INFO", "EXPERIENCE", "SKILLS"]):
                            valid_entries.append(entry)
                    if valid_entries:
                        result["education"] = valid_entries
                    
                # Process Experience
                elif "experience" in section_name or "employment" in section_name or "work" in section_name:
                    ...
                    for entry in experience_entries:
                        ...
                        if entry.get("company") and re.search(r'\b(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4}\s+to\s+', entry.get("company")):
                            # This is a date, not a company name
                            date_match = re.search(r'\b((?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4})\s+to\s+((?:Current|Present|(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4}))', entry.get("company"))
                            if date_match:
                                entry["dates"] = f"{date_match.group(1)} - {date_match.group(2)}"
                                entry["company"] = ""
                                entry_cleaned = True
                        ...
                        if entry.get("company") and re.match(r'^[A-Z]$', entry.get("company").strip()):
                            entry["company"] = ""
                            entry_cleaned = True
                            
                        # Fix case where company name starts with "TO" followed by a date
                        if entry.get("company") and entry.get("company").strip().startswith("TO"):
                        ...
                        if entry.get("company") and not entry_cleaned:
                            ...
                            company_match = re.search(r'(?:at|with|for)?\s+([A-Z][a-zA-Z0-9\s&\.]+(?:Inc|LLC|Ltd|Corp|Company|Technologies|Systems|Group|Media|Services|Solutions|Associates))', company_name)
                            if company_match:
                                entry["company"] = company_match.group(1).strip()
                                entry_cleaned = True
                            else:
                                # Try another pattern - Company Name followed by location
                                company_match = re.search(r'([A-Z][a-zA-Z0-9\s&\.]+)(?:\s+[\u00ef\u00bc\u200b]+\s+City\s*,\s*State)', company_name)
                                if company_match:
                                    entry["company"] = company_match.group(1).strip()
                                    entry_cleaned = True
                                else:
                                    # If it starts with letters like "J", "C", "F" followed by "anuary", "ompany", etc.
                                    if re.match(r'^[A-Z](?:anuary|ebruary|arch|pril|ay|une|uly|ugust|eptember|ctober|ovember|ecember)', company_name):
                                        # This is a partial date
                                        entry["company"] = ""
                                        entry_cleaned = True
                                    # If it's the first entry and has key words like "System Architect" or "SVP", it's likely a title
                                    elif len(valid_entries) == 0 and re.search(r'(architect|chief|director|SVP|VP|manager|executive|officer)', company_name, re.IGNORECASE):
                                        # Move this to title if title is empty
                                        if not entry.get("title"):
                                            # Extract a reasonable title
                                            title_match = re.search(r'([A-Z][a-zA-Z\s]+(?:Architect|Chief|Director|SVP|VP|Manager|Executive|Officer)[a-zA-Z\s]*)', company_name)
                                            if title_match:
                                                ...
                                                entry["title"] = company_name[:50].strip()
                                                entry["company"] = ""
                                                entry_cleaned = True
                        ...
                        if entry.get("description"):
                            ...
                            if re.search(r'([A-Z])\s*$', entry.get("description")):
                                ...
                                entry["description"] = re.sub(r'([A-Z])\s*$', '', entry.get("description")).strip()
                            ...
                            if "==========" in entry.get("description"):
                                ...
                                entry["description"] = entry.get("description").split("==========")[0].strip()
                        ...
                        if (not entry.get("title") or entry.get("title") == "") and entry.get("description"):
                            # Common job titles at the beginning of descriptions
                            title_match = re.match(r'^((?:Chief|Senior|Principal|Lead|Director|Manager|VP|SVP|EVP|Executive|Head|President|CEO|CTO|CIO|CFO|COO|Architect|Engineer|Developer|Consultant|Analyst|Specialist|Administrator)[\w\s]+?(?:Architect|Engineer|Developer|Manager|Director|Officer|Executive|Chief|Lead|Head|Specialist|Analyst|Consultant|Administrator))\s', entry.get("description"))
                            if title_match:
                                entry["title"] = title_match.group(1).strip()
                                # Remove from beginning of description
                                entry["description"] = entry.get("description")[len(entry["title"]):].strip()
                        
                        # If no company name, try to extract from description
                        if (not entry.get("company") or entry.get("company") == "") and entry.get("description"):
                            ...
                            if len(desc_lines) >= 1:
                                ...
                                if not company_line.lower().startswith(('i ', 'my ', 'the ', 'this ', 'they ', 'we ', 'as ', 'in ', 'at ', 'to ', 'resp', 'creat', 'feat', 'feat')):
                                    if "City , State" in company_line:
                                        # This is likely "Company Name City, State"
                                        company_name = company_line.split("City , State")[0].strip()
                        ...
                        if ((entry.get("company") and entry.get("company").strip()) or 
                            (entry.get("title") and entry.get("title").strip()) or 
                            (entry.get("dates") and entry.get("dates").strip())) and \
                            len(entry.get("description", "").strip()) > 30:
                            valid_entries.append(entry)
                ...
                # Process Skills
                    ...
                    valid_skills = []
                    for skill in skills:
                        if not any(kw in skill.upper() for kw in ["CERTIFICATIONS", "LANGUAGES", "PROJECTS", "PERSONAL", "EDUCATION"]):
                ...
                # Process Languages
                    ...
                    for lang in languages:
                        if not any(kw in lang.get("language", "").upper() for kw in ["CERTIFICATIONS", "PROJECTS", "SKILLS"]):
                ...
                # Process Certifications
                    ...
                    known_certifications = [
                        "MCSE", "CCNA", "CCNP", "CCIE", "AWS Certified", "Azure Certified", 
                        "CompTIA", "PMP", "ITIL", "Certified Scrum", "Six Sigma", "CEH",
                        "CISSP", "Security+", "Network+", "A+", "CSM", "CISA", "CISM",
                        "Google Certified", "Oracle Certified", "IBM Certified", "SAP Certified",
                        "PRINCE2", "TOGAF", "CFA", "CPA", "PMI-ACP", "Certified Ethical Hacker",
                        "GIAC", "Salesforce Certified"
                    ]
                    ...
                    degree_matches = re.finditer(degree_pattern, section_content, re.IGNORECASE)
                    ...
                    if not valid_certs:
                        ...
                        for cert in known_certifications:
                            if re.search(r'\b' + re.escape(cert) + r'\b', section_content, re.IGNORECASE):
                                # Find the full certification phrase
                                context = re.search(r'[^.!?:]{0,50}\b' + re.escape(cert) + r'\b[^.!?:]{0,30}', section_content, re.IGNORECASE)
                                if context:
                                    cert_text = context.group(0).strip()
                                    # Only add if it's not too long and doesn't contain suspicious text
                                    if 5 <= len(cert_text) <= 50 and not any(w in cert_text.lower() for w in ['companies', 'clients', 'technologies', 'technical', 'systems to', 'markets', 'production']):
                                        valid_certs.append({
                                            "name": cert_text,
                                            "date": ""
                                        })
                    ...
                    for cert in valid_certs:
                        ...
                        if cert_name.lower().startswith(('and ', 'the ', 'of ', 'in ', 'to ', 'with ', 'over ', 'these ', 'many ', 'on ', 'at ', 'by ', 'for ')):
                            continue
                            
                        # Skip if it contains suspicious words that indicate it's not a certification
                        if any(word in cert_name.lower() for word in ['companies', 'clients', 'executive', 'decision', 'relationships', 'technical', 'system', 'handover', 'markets', 'digital']):
                            continue
                            
                        # Only add entries that look like actual certifications or degrees
                        if (re.search(r'\b(?:Certified|Certificate|Degree|License|Bachelor|Master|MBA|PhD|BSc|MSc|BA|BS|MS|MA)\b', cert_name, re.IGNORECASE) or
                            re.search(r'\b(?:MCSE|CCNA|CCNP|CCIE|AWS|Azure|CompTIA|PMP|ITIL|CSM|CEH|CISSP|Security\+|Network\+|A\+)\b', cert_name, re.IGNORECASE)):
                    ...
                    if filtered_certs:
                        ...
                        matches = re.finditer(education_pattern, section_content, re.IGNORECASE)
                        for match in matches:
                ...
                elif "project" in section_name and hasattr(self, "_parse_projects_section"):
                    projects = self._parse_projects_section(section_content)
                    if projects:
                        result["projects"] = projects
                    
                elif "interest" in section_name and hasattr(self, "_parse_interests_section"):
        ...
        if section_count == 0:
            logger.warning("No valid sections found in segmented text, falling back to traditional parsing")
            return self._parse_traditional(segmented_text)
        ...
        for section_name, section_data in result.items():
            ...
            logger.debug(f"Section {section_name} is empty, attempting traditional extraction")
            try:
                if section_name == "personal_info" and not any(result["personal_info"].values()):
                    result["personal_info"] = self.extract_personal_info(segmented_text)
                    
                elif section_name == "professional_summary" and not result["professional_summary"]:
                    result["professional_summary"] = self.extract_professional_summary(segmented_text)
                    ...
                    result["certifications"] = self._extract_certifications(segmented_text)
            except Exception as e:
                logger.error(f"Error in traditional extraction for {section_name}: {str(e)}")
        ...
        if self._is_technology_cv(segmented_text):
            ...
            if not result["experience"] or len(result["experience"]) < 2:
                result["experience"] = self._extract_healthcare_experience(segmented_text)
        ...
        if not any(result.values()) or (not any(result["personal_info"].values()) and 
                                        not result["experience"] and 
                                        not result["education"] and 
                                        not result["skills"]):
            logger.warning("Segmented parsing produced insufficient results, falling back to traditional")
            return self._parse_traditional(segmented_text)
    ...
    def _parse_personal_info_section(self, section_text):
        ...
        email_matches = re.findall(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', section_text)
        if email_matches:
        ...
        location_indicators = ["Address:", "Location:", "City:", "Based in:", "Living in:"]
        for line in lines:
            line = line.strip()
            # Check if line contains location indicators
            if any(indicator in line for indicator in location_indicators):
                info['location'] = line.split(':', 1)[1].strip() if ':' in line else line
                break
    ...
    def _parse_education_section(self, section_text):
        ...
        for entry in entries:
            ...
            degree_keywords = ["Bachelor", "Master", "Ph.D", "PhD", "Doctorate", "B.S.", "M.S.", "B.A.", "M.A.", "MBA"]
            for line in lines[1:]:
            ...
            for line in lines:
                if re.search(date_pattern, line, re.IGNORECASE):
                    education['dates'] = re.search(date_pattern, line, re.IGNORECASE).group().strip()
                    break
            ...
            if len(lines) > 1:
                ...
                for line in lines[1:]:
                    if line.strip() and not any([
                        ...
                        education['field_of_study'] and f"in {education['field_of_study']}" in line
                    ]):
    ...
    def _extract_language_entry(self, text):
        ...
        proficiency_levels = [
            'Native', 'Fluent', 'Proficient', 'Intermediate', 'Conversational', 'Basic',
            'C2', 'C1', 'B2', 'B1', 'A2', 'A1'
        ]
    ...
    def _sanitize_text(self, text: str) -> str:
        ...
        if len(text) > max_chars:
            logger.warning(f"Truncating text from {len(text)} to {max_chars} chars for LLM processing")
            text = text[:max_chars]
            
        return text
        
    def _post_process_aviation_cv(self, data: Dict[str, Any], text: str) -> Dict[str, Any]:
        """
        ...
        for pattern in license_patterns:
            for license_match in re.finditer(pattern, text, re.IGNORECASE):
                ...
                if is_new:
                    ...
                    logger.debug(f"Added aviation license to certifications: {license_text}")
        
        return data
        
    def _post_process_banking_cv(self, data: Dict[str, Any], text: str) -> Dict[str, Any]:
        """
        ...
        finance_skills = [
            ...
            "Budgeting", "Forecasting", "Risk Management", "Investment Banking",
            "Asset Management", "Wealth Management", "Portfolio Management",
            "Equity Research", "Fixed Income", "Derivatives", "Options", "Futures",
            "Swaps", "Bloomberg Terminal", "Reuters", "Capital Markets", "M&A",
            "Mergers and Acquisitions", "Valuation", "DCF", "Discounted Cash Flow",
            "NPV", "IRR", "Cost Accounting", "Management Accounting", "Tax Planning",
            "Auditing", "Compliance", "AML", "Anti-Money Laundering", "KYC", "Know Your Customer"
        ]
            ...
            for section_name, patterns in section_headers.items():
                for pattern in patterns:
                    if re.search(pattern, line):
                        ...
                        section_positions[section_name] = (current_line_index, -1)  # -1 means end not determined yet
                        break
    ...
    def _is_aviation_cv(self, text: str) -> bool:
        ...
        aviation_terms = [
            "pilot", "flight", "aircraft", "aviation", "helicopter", "faa", "airline",
            "cockpit", "captain", "first officer", "cabin crew", "navigator", "airbus", "boeing",
            "flight hours", "type rating", "air traffic", "atpl", "cpl", "ppl", "ifr", "vfr"
        ]
    ...
    def _is_banking_cv(self, text: str) -> bool:
        ...
        banking_terms = [
            "accounting", "accountant", "financial", "bank", "investment", "analyst", "finance",
            "portfolio", "credit", "loan", "asset", "wealth management", "risk assessment",
            "trading", "securities", "treasury", "corporate finance", "m&a", "audit", "compliance",
            "regulatory", "fintech", "banking", "mortgage", "underwriting"
        ]
    ...
    def _extract_healthcare_experience(self, text):
        ...
        if len(experiences) < 2:
            ...
            experience_section = self._extract_text_section(text, 
            ...
            if experience_section:
                ...
                segments = re.split(r"(\b(?:19|20)\d{2}\s*-\s*(?:19|20)\d{2}|\b(?:19|20)\d{2}\s*-\s*present\b|\b(?:19|20)\d{2}\s*to\s*(?:19|20)\d{2}|\b(?:19|20)\d{2}\s*to\s*present\b)", experience_section)
                ...
                for i in range(1, len(segments)-1, 2):
                    ...
                    for pattern in role_patterns:
                        ...
                        if role_match:
                            # Look for full title
                            title_pattern = r"(?i)([A-Z][a-z]+ " + role_match.group(1) + r"|\b" + role_match.group(1) + r" [A-Z][a-z]+)"
                    ...
                    for pattern in org_patterns:
                        ...
                        if org_match:
                            ...
                            full_org_pattern = r"(?i)([A-Z][a-zA-Z\s]+ " + org_match.group(1) + r"|\b" + org_match.group(1) + r" [A-Z][a-zA-Z\s]+)"
                    ...
                    if role and date_range:
                        experiences.append({
                            "title": role.strip(),
                            "company": organization.strip() if organization else "Unknown",
        ...
        for pattern in all_patterns:
            ...
            for match in matches:
                ...
                if company and len(company) > 2:  # Ensure it's not just a short acronym
                    ...
                    company = re.sub(r'[,.]$', '', company)  # Remove trailing commas/periods
                    
                    # Don't return likely false positives (common words)
                    if company.lower() in ["the", "and", "inc", "llc", "ltd", "corporation", "company"]:
                        continue
        ...
        for pattern in all_patterns:
            ...
            for match in matches:
                ...
                if title and len(title) > 3:  # Ensure it's not too short
                    ...
                    title = re.sub(r'[,.]$', '', title)  # Remove trailing commas/periods
                    
                    # Don't return likely false positives (common words)
                    if title.lower() in ["position", "title", "role", "job", "work", "employee", "staff"]:
                        continue
    ...
    def _identify_cv_sections_safe(self, text, timeout=5.0):
        ...
        lines = text.split('\n')
        if len(lines) > 1000:
            self.logger.warning(f"CV has too many lines ({len(lines)}), limiting to 1000")
            lines = lines[:1000]
    ...
    def _extract_experience_efficient(self, text, sections=None):
        ...
        if sections and 'experience' in sections:
            start_pos, end_pos = sections['experience']
            experience_text = text[start_pos:end_pos]
            self.logger.debug(f"Using pre-identified experience section ({len(experience_text)} chars)")
        else:
            ...
            for pattern in experience_patterns:
                ...
                if experience_match:
                    ...
                    self.logger.debug(f"Found experience section via pattern matching ({len(experience_text)} chars)")
                    break
            ...
            if not experience_text:
                ...
                for p in paragraphs[2:]:
                    ...
                    if (re.search(job_title_indicators, p) and re.search(date_indicators, p)) or len(p.split('\n')) >= 3:
                        experience_paragraphs.append(p)
                
                if experience_paragraphs:
                    experience_text = '\n\n'.join(experience_paragraphs)
                    self.logger.debug(f"Extracted potential experience text from paragraphs ({len(experience_text)} chars)")
                else:
                    ...
                    experience_text = text[start_pos:end_pos]
                    self.logger.debug(f"Using middle portion of text for experience extraction ({len(experience_text)} chars)")
        ...
        for pattern in job_entry_patterns:
            ...
            for match in matches:
                ...
                for marker in job_markers:
                    marker_matches = list(re.finditer(marker, experience_text[start_pos:]))
                    if marker_matches:
                        potential_next = marker_matches[0]
                        if not next_marker_match or start_pos + potential_next.start() < next_marker_match[1]:
                            next_marker_match = (potential_next, start_pos + potential_next.start())
        ...
        if not entries:
            ...
            for i in range(len(block_start_positions)):
                ...
                end = block_start_positions[i+1] if i < len(block_start_positions) - 1 else len(experience_text)
            ...
            for block in blocks:
                ...
                for pattern in company_patterns:
                    ...
                    if company_match:
                        ...
                        if (len(potential_company) < 50 and 
                            not re.search(r'\b(?:experience|skill|qualification|education|language)\b', potential_company, re.IGNORECASE)):
                ...
                for pattern in title_patterns:
                    ...
                    if title_match:
                        ...
                        # Validate - should not be too long or contain unwanted terms
                        if (len(potential_title) < 50 and 
                            not re.search(r'\b(?:company|inc|llc|ltd)\b', potential_title, re.IGNORECASE)):
                ...
                if entry.get("dates"):
                    ...
                    description = re.sub(r'(?:Company|Title)\s*[:\-–—]\s*', '', description)
                    description = re.sub(r'^\s*[,.;:]\s*', '', description)  # Remove leading punctuation
                    description = re.sub(r'City\s*[,.]?\s*State', '', description)  # Remove placeholder text
                    
                    entry["description"] = description.strip()
                
                # Add entry if it has enough information
                if entry.get("dates") and entry.get("description") and len(entry["description"]) > 30:
        ...
        for entry in entries:
            ...
            if entry.get("company"):
                entry["company"] = re.sub(r'Company\s+Name\s*[:\-–—]', '', entry["company"])
            ...
            if entry.get("dates"):
                ...
                entry["dates"] = re.sub(r'to', '-', entry["dates"])   # Standardize separator
            
            # Make sure we have a good description
            if entry.get("description"):
                # Remove any remaining markers or labels
                entry["description"] = re.sub(r'Description\s*[:\-–—]', '', entry["description"])
                entry["description"] = re.sub(r'Responsibilities\s*[:\-–—]', '', entry["description"])
                entry["description"] = re.sub(r'^\s*[,.;:]\s*', '', entry["description"])
        ...
        for entry in entries:
            ...
            has_description = bool(entry.get("description") and len(entry.get("description", "")) > 30)
        ...
        self.logger.debug(f"Extracted {len(final_entries)} experience entries efficiently")
    ...
    def _segment_with_deepseek(self, text, timeout=60):
        ...
        try:
            ...
            # Log completion
            self.logger.info(f"DeepSeek segmentation completed in {elapsed_time:.2f} seconds")
            ...
            if segmented_text and "==========" in segmented_text:
                ...
                if placeholder_count > 3:
                    self.logger.warning(f"DeepSeek segmentation returned too many placeholders ({placeholder_count})")
                    return None
                ...
                if valid_sections < 2:
                    self.logger.warning(f"DeepSeek segmentation returned only {valid_sections} valid sections")
                    return None
                
                self.logger.info(f"DeepSeek segmentation successful with {section_count} sections")
                return segmented_text
            else:
                self.logger.warning("DeepSeek segmentation failed or returned invalid format")
                return None
        except Exception as e:
    ...
    def extract_skills(self, text, sections=None):
        ...
        technical_skills = [
            "Python", "Java", "JavaScript", "C++", "C#", "SQL", "HTML", "CSS", "AWS", 
            "Azure", "GCP", "Docker", "Kubernetes", "Linux", "Windows", "macOS", "iOS", 
            "Android", "Git", "Node.js", "React", "Angular", "Vue.js", "Django", 
            "Flask", "Spring", "TensorFlow", "PyTorch", "R", "MATLAB", "Tableau", 
            "Power BI", "Excel", "Word", "PowerPoint", "Photoshop", "Illustrator", 
            "JIRA", "Confluence", "Trello", "Agile", "Scrum", "Kanban", "CI/CD", 
            "DevOps", "CloudFormation", "Terraform", "Jenkins", "GitHub", "GitLab", 
            "BitBucket", "REST API", "GraphQL", "microservices", "NoSQL", "MongoDB", 
            "PostgreSQL", "MySQL", "Oracle", "Redis", "Elasticsearch", "Hadoop", 
            "Spark", "Kafka", "RabbitMQ", "MQTT", "WebSockets", "OAuth", "JWT", 
            "SAML", "LDAP", "ActiveDirectory", "Nginx", "Apache", "IIS", "Tomcat", 
            "JBoss", "WebLogic", "WebSphere", "VMware", "VirtualBox", "Hyper-V", 
            "OpenStack", "NetApp", "SAN", "NAS", "S3", "EBS", "EFS", "Route53", 
            "CloudFront", "CloudWatch", "IAM", "VPC", "EC2", "RDS", "DynamoDB", 
            "Lambda", "API Gateway", "SQS", "SNS", "Kinesis", "EMR", "Redshift", 
            "Snowflake", "IPTV", "OTT", "MPLS", "VOD", "DVB", "DVB-S", "DVB-T", 
            "DTH", "Digital Media", "Video Encoding", "Video Transcoding"
        ]
        ...
        soft_skills = [
            ...
            "Interpersonal Skills", "Active Listening", "Coaching", "Mentoring", 
            "Persuasion", "Storytelling", "Project Management", "Strategic Planning"
        ]
        
        # Define industry-specific skills
        industry_skills = {
            "IT": [
                "Network Administration", "Database Management", "Cloud Computing", 
                "Information Security", "System Administration", "Technical Support", 
                "Data Engineering", "Software Development", "Web Development", 
                "Mobile Development", "UI/UX Design", "QA Testing", "IT Service Management"
            ],
            "Media": [
                "Broadcasting", "Video Production", "Content Creation", "Film Production", 
                "Publishing", "Journalism", "Content Strategy", "Digital Marketing", 
                "Social Media Management", "SEO", "SEM", "Public Relations", 
                "Copywriting", "Photography", "Animation", "Graphic Design", 
                "UX Writing", "Content Distribution", "Podcasting", "Livestreaming",
                "IPTV", "OTT", "DVB", "DVB-S", "DVB-T", "DTH", "MPLS", "VOD",
                "Television", "TV", "Cable", "Satellite", "Media"
            ],
            "Finance": [
                "Financial Analysis", "Financial Reporting", "Budgeting", "Forecasting", 
                "Accounting", "Taxation", "Audit", "Risk Management", "Investment Banking", 
                "Asset Management", "Wealth Management", "Financial Planning", 
                "Corporate Finance", "Mergers & Acquisitions", "Private Equity", 
                "Venture Capital", "Treasury", "Credit Analysis", "Financial Modeling"
            ]
        }
        ...
        for section_name, section_content in sections.items():
            if any(kw in section_name.lower() for kw in ["skill", "expertise", "competenc", "proficienc", "technical", "technology", "technologies"]):
        ...
        if skills_section_content:
            ...
            for extract_func in skilled_functions:
                try:
                    ...
                    if extracted_skills:
                        self.logger.debug(f"Extracted {len(extracted_skills)} skills using {extract_func.__name__}")
                        result.extend(extracted_skills)
                except Exception as e:
                    self.logger.error(f"Error extracting skills with {extract_func.__name__}: {e}")
        
        # Extract from the entire CV if needed
        if not result:
            self.logger.debug("No skills found in dedicated sections, trying to extract from the entire CV")
            
            # Try to extract skills from technology-related sentences 
            tech_sentences = []
            
            # Look for sentences containing tech keywords
            tech_keywords = ["technical", "technology", "technologies", "proficient", "skilled", "expertise", "experience with", "knowledge of", "tools", "platforms", "familiar with", "working with"]
            ...
            if tech_sentences:
                ...
                for extract_func in skilled_functions:
                    try:
                        ...
                        if extracted_skills:
                            self.logger.debug(f"Extracted {len(extracted_skills)} skills from tech sentences using {extract_func.__name__}")
                            result.extend(extracted_skills)
                    except Exception as e:
                        self.logger.error(f"Error extracting skills from tech sentences with {extract_func.__name__}: {e}")
                ...
                for paragraph in paragraphs:
                    ...
                    for extract_func in skilled_functions:
                        try:
                            ...
                            if extracted_skills:
                                self.logger.debug(f"Extracted {len(extracted_skills)} skills from paragraph using {extract_func.__name__}")
                                result.extend(extracted_skills)
                        except Exception as e:
                            continue
        ...
        final_skills = []
        non_skills = ["Responsibilities", "Accomplishments", "Experience", "Tasks", "References",
                     "Contact", "Education", "Phone", "Email", "Address", "Location",
                     "First Name", "Last Name", "Full Name", "Title", "Summary", "Profile",
            ...
            for match in matches:
                ...
                for skill in potential_skills:
                    ...
                    if (3 <= len(skill) <= 50 and 
                        not re.search(r'\d{4}|\n|experience|years|level|proficiency', skill, re.IGNORECASE)):
                        skills.append(skill)
        ...
        for line in lines:
            ...
            if len(comma_items) >= 3:
                ...
                for item in comma_items[:5]:  # Check first 5 items
                    ...
                    if re.search(r'(?:python|java|javascript|sql|html|css|react|node|git|aws|docker|kubernetes|angular|vue|php|c\+\+|c#|ruby|go|swift|kotlin|typescript|mongodb|postgresql|mysql|redis|jenkins|terraform)', item, re.IGNORECASE):
                        tech_count += 1
                ...
                if tech_count >= 2:
                    for item in comma_items:
                        ...
                        if (3 <= len(item) <= 30 and 
                            not re.search(r'\d{4}|experience|years', item, re.IGNORECASE)):
                            skills.append(item)
