        ...
        try:
            ...
            try:
                ...
                response = loop.run_until_complete(
                    self.llama_service.generate_with_system_prompt(
                        system_prompt, user_prompt, timeout=60
                    )
                )
            except RuntimeError:
...
IMPORTANT: Return ONLY the JSON object, no explanations, no markdown formatting."""
    ...
    def _validate_and_clean(self, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Validated CV data: {len(result['experience'])} experience, {len(result['skills'])} skills, {len(result['education'])} education"
        )
        return result
    ...
    def _fallback_parse(self, cv_text: str) -> Dict[str, Any]:
            ...
            if line.upper() in [
                ...
                for j in range(i + 1, min(i + 6, len(lines))):
                    ...
                    if candidate_line and self._is_likely_name(candidate_line):
                        ...
                        logger.info(
                            f"Extracted name from CONTACT section: {result['personal_info']['first_name']} {result['personal_info']['last_name']}"
                        )
                        break
        ...
        if not result["personal_info"]["first_name"]:
            for i, line in enumerate(lines[:10]):
                ...
                if line and self._is_likely_name(line):
                    ...
                    result["personal_info"]["last_name"] = " ".join(parts[1:])
                    logger.info(
                        f"Extracted name from beginning: {result['personal_info']['first_name']} {result['personal_info']['last_name']}"
                    )
                    break
        ...
        if not result["personal_info"]["first_name"]:
            for i, line in enumerate(lines):
                ...
                if "@" in line or any(
                    pattern in line
                    for pattern in ["+44", "+1", "07", "phone", "mobile", "tel"]
                ):
                    ...
                    for offset in [-2, -1, 1, 2]:
                        ...
                        if 0 <= check_index < len(lines):
                            ...
                            if candidate_line and self._is_likely_name(candidate_line):
                                parts = candidate_line.split()
                                result["personal_info"]["first_name"] = parts[0]
                                result["personal_info"]["last_name"] = " ".join(
                                    parts[1:]
                                )
                                logger.info(
                                    f"Extracted name near contact info: {result['personal_info']['first_name']} {result['personal_info']['last_name']}"
                                )
                                break
                    if result["personal_info"]["first_name"]:
                        break
        ...
        for pattern in experience_patterns:
            ...
            if match:
                ...
                for block in job_blocks:
                    ...
                    title_company_match = re.match(
                        r"^(.+?)\s*\|\s*(.+?)(?:\s{2,}(.+?))?$", first_line
                    )
                    if title_company_match:
                        job_title = title_company_match.group(1).strip()
                        company_info = title_company_match.group(2).strip()
                        date_info = (
                            title_company_match.group(3).strip()
                            if title_company_match.group(3)
                            else ""
                        )

                        # Clean up company name (remove trailing dates if any)
                        company = re.sub(
                            r"\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|\d{4}|\d{1,2}/).*$",
                            "",
                            company_info,
                        ).strip()
                                ...
                                result["experience"].append(
                                    {
                                        ...
                                        "location": "",
                                        "current": end_date.lower()
                                        in ["present", "current"],
                                    }
                                )
                                logger.info(
                                    f"Extracted job: {job_title} at {company} ({start_date} - {end_date})"
                                )
                break
            ...
            if match:
                ...
                for block in edu_blocks:
                    ...
                    if degree_school_match:
                        ...
                        date_info = (
                            degree_school_match.group(3).strip()
                            if degree_school_match.group(3)
                            else ""
                        )
                        ...
                        degree_field_match = re.match(
                            r"^(BSc|MSc|BA|MA|PhD|Diploma|Certificate|Degree)\s+(?:in\s+)?(.+)",
                            degree_info,
                        )
                    ...
                    for line in lines[1:]:
                        ...
                        if not location and any(
                            ...
                            for keyword in ["london", "uk", "university", "college"]
                        ):
                            location = line
                    ...
                    if degree and school:
                        ...
                        clean_field = (
                            re.sub(r"\s+", " ", field_of_study).strip()
                            if field_of_study
                            else ""
                        )
                        ...
                        clean_school = re.sub(
                            r"^(University of |College of )", "", clean_school
                        )
                        clean_school = re.sub(
                            r"( University| College)$", "", clean_school
                        ) + (" University" if "University" in school else "")
                        ...
                        if len(clean_degree) > 2 and len(clean_school) > 2:
                            ...
                            logger.info(
                                f"Extracted education: {clean_degree} in {clean_field} from {clean_school} ({end_date})"
                            )
                break

        logger.info(
            f"Fallback parsing extracted: email={bool(result['personal_info']['email'])}, phone={bool(result['personal_info']['phone'])}, name={bool(result['personal_info']['first_name'])}, experience={len(result['experience'])}, education={len(result['education'])}"
        )
        return result
    ...
    def _is_likely_name(self, line: str) -> bool:
        ...
        if any(
            indicator in line
            for indicator in ["@", "www", ".com", "http", "©", "|", ":", "+44", "+1"]
        ):
            return False
