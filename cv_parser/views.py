...
from ai_cv_parser.file_security import validate_uploaded_file, log_security_event
...
class CVParserViewSet(viewsets.ViewSet):
        ...
        try:
            ...
            try:
                ...
                parsed_cv = ParsedCV.objects.create(
                    user=request.user, original_file=file, parsed_data=result
                )

                return Response({"id": parsed_cv.id, "data": result})
    ...
    def transfer_to_writer(self, request):
        ...
        try:
            ...
            logger.info(
                f"CV transfer request from user {request.user.username} (ID: {request.user.id})"
            )
            ...
            if not parsed_data:
                logger.warning(
                    f"No parsed data provided in transfer request by user {request.user.username}"
                )
                return Response(
            ...
            cv_writer = CvWriter.objects.create(
                user=request.user, title="CV from Parser", status="active"
            )
            ...
            if parsed_data.get("experience"):
                logger.info(
                    f"Adding {len(parsed_data.get('experience'))} experiences to CV {cv_writer.id}"
                )
                for i, exp_data in enumerate(parsed_data.get("experience")):
                    logger.info(
                        f"Adding experience {i+1}: {exp_data.get('job_title', '')} at {exp_data.get('company', '')}"
                    )
            ...
            if parsed_data.get("education"):
                logger.info(
                    f"Adding {len(parsed_data.get('education'))} education entries to CV {cv_writer.id}"
                )
                for i, edu_data in enumerate(parsed_data.get("education")):
                    logger.info(
                        f"Adding education {i+1}: {edu_data.get('degree', '')} at {edu_data.get('school', '')}"
                    )
            ...
            if parsed_data.get("skills"):
                ...
                for i, skill_data in enumerate(parsed_data.get("skills")):
                    if isinstance(skill_data, dict):
                        ...
                        logger.info(
                            f"Adding skill {i+1}: {skill_name} (from dictionary)"
                        )
            ...
            if parsed_data.get("languages"):
                ...
                for i, lang_data in enumerate(parsed_data.get("languages")):
                    if isinstance(lang_data, dict):
                        ...
                        logger.info(
                            f"Adding language {i+1}: {lang_name} (from dictionary)"
                        )
            ...
            logger.info(
                f"Data successfully transferred to CV Writer with ID: {cv_writer.id}"
            )
            return Response(
                {
    ...
    def extract_sections(self, request):
        ...
        try:
            ...
            logger.info(
                f"Authenticated user {request.user.username} (ID: {request.user.id}) is attempting to extract sections"
            )
            ...
            if not text:
                logger.warning(
                    f"No text provided in extract_sections request by user {request.user.username}"
                )
                return Response(
            ...
            # Log the request
            logger.info(
                f"Section extraction request for text of length {len(text)} by user {request.user.username}"
            )
            ...
            logger.info(
                f"Successfully extracted {section_count} sections for user {request.user.username}"
            )
...
class ParseCVView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            validation_result = validate_uploaded_file(cv_file)
            ...
            if not validation_result["valid"]:
                ...
                log_security_event(
                    "authenticated_upload_rejected",
                    {
                        ...
                        "size": cv_file.size,
                        "mime_type": getattr(cv_file, "content_type", "unknown"),
                    },
                    ip_address,
                )

                logger.warning(
                    f"Security validation failed for user {request.user.id}: {validation_result['error']}"
                )
            ...
            cv_file.name = validation_result["sanitized_name"]
            logger.info(
                f"File validated successfully. Original: {original_name}, Sanitized: {cv_file.name}"
            )

            # Additional basic size check (redundancy)
            ...
            # Extract file type from sanitized filename
            ...
            try:
                ...
                return Response(
                    {"cv_id": parsed_cv.id, "message": "CV parsed successfully"},
                ...
                logger.error(
                    f"Connection timeout error while parsing document: {str(e)}"
                )
                os.unlink(tmp_file_path)  # Clean up
                return Response(
                    {
...
def cv_from_parser(request):
    ...
    try:
        ...
        try:
            ...
            if not parsed_cv_id:
                return JsonResponse({"error": "Parsed CV ID is required"}, status=400)
        ...
        name_parts = (
            parsed_data.get("professional_summary", "").split(" ")[:2]
            if parsed_data.get("professional_summary")
            else ["", ""]
        )
        ...
        for lang_name in parsed_data.get("languages", []):
            if lang_name:
                Language.objects.create(
                    cv=cv,
                    language=(
                        lang_name if isinstance(lang_name, str) else str(lang_name)
                    ),
        ...
        for cert_data in parsed_data.get("certifications", []):
            if isinstance(cert_data, dict):
                Certification.objects.create(
                    ...
                    certificate_date=(
                        parse_date(cert_data.get("year", ""))
                        if cert_data.get("year")
                        else None
                    ),
                )
            elif cert_data:
                Certification.objects.create(
                    cv=cv,
                    certificate_name=(
                        cert_data if isinstance(cert_data, str) else str(cert_data)
                    ),
                )

        # Return the new CV ID
        return JsonResponse(
            {"success": True, "cv_id": cv.id, "message": "CV created successfully"}
        )
...
def download_cv(request, cv_id):
    ...
    try:
        ...
        styles.add(
            ParagraphStyle(
                name="Heading1", fontName="Helvetica-Bold", fontSize=16, spaceAfter=12
            )
        )
        styles.add(
            ParagraphStyle(
        ...
        styles.add(
            ParagraphStyle(
                name="Normal", fontName="Helvetica", fontSize=12, spaceAfter=6
            )
        )
        styles.add(
            ParagraphStyle(
                name="ListItem", fontName="Helvetica", fontSize=12, leftIndent=20
            )
        )
            ...
            if isinstance(cv_data["skills"], list):
                ...
                for skill in cv_data["skills"]:
                    if isinstance(skill, str):
                        skills_list.append(
                            ListItem(Paragraph(skill, styles["ListItem"]))
                        )
                        ...
                        skills_list.append(
                            ListItem(Paragraph(skill_text, styles["ListItem"]))
                        )
                ...
                if skills_list:
                    ...
                    content.append(
                        Paragraph("No specific skills listed", styles["Normal"])
                    )
            else:
                            ...
                            if isinstance(responsibilities, list):
                                for resp in responsibilities:
                                    if isinstance(resp, str):
                                        resp_list.append(
                                            ListItem(
                                                Paragraph(resp, styles["ListItem"])
                                            )
                                        )

                                if resp_list:
                                    content.append(
                                        ListFlowable(resp_list, bulletType="bullet")
                                    )
                            else:
                                content.append(
                                    Paragraph(str(responsibilities), styles["Normal"])
                                )
                    else:
            ...
            if isinstance(cv_data["certifications"], list):
                ...
                for cert in cv_data["certifications"]:
                    if isinstance(cert, dict):
                        ...
                        cert_list.append(
                            ListItem(Paragraph(cert_text, styles["ListItem"]))
                        )
                ...
                if cert_list:
                    ...
                    content.append(
                        Paragraph("No specific certifications listed", styles["Normal"])
                    )
            else:
                content.append(
                    Paragraph(str(cv_data["certifications"]), styles["Normal"])
                )
            ...
            if isinstance(cv_data["languages"], list):
                ...
                for lang in cv_data["languages"]:
                    if isinstance(lang, str):
                        ...
                        lang_list.append(
                            ListItem(Paragraph(lang_text, styles["ListItem"]))
                        )
                ...
                if lang_list:
                    ...
                    content.append(
                        Paragraph("No specific languages listed", styles["Normal"])
                    )
            else:
        ...
        if "contact_info" in cv_data and cv_data["contact_info"]:
            ...
            if isinstance(cv_data["contact_info"], dict):
                ...
                if "email" in contact_info:
                    content.append(
                        Paragraph(f"Email: {contact_info['email']}", styles["Normal"])
                    )

                if "phone" in contact_info:
                    content.append(
                        Paragraph(f"Phone: {contact_info['phone']}", styles["Normal"])
                    )

                if "location" in contact_info:
                    content.append(
                        Paragraph(
                            f"Location: {contact_info['location']}", styles["Normal"]
                        )
                    )
            else:
                content.append(
                    Paragraph(str(cv_data["contact_info"]), styles["Normal"])
                )
