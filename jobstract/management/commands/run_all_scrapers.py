...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"===== Starting all job scrapers at {start_time.strftime('%Y-%m-%d %H:%M:%S')} =====")
        ...
        for scraper in scrapers:
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS(f"✓ {scraper_name} completed successfully"))
                ...
                self.stdout.write(self.style.ERROR(f"✗ {scraper_name} failed: {error_msg}"))
        ...
        for result in results:
            ...
            self.stdout.write(status_style(f"{status_symbol} {result['scraper']}: {result['message']}"))
