...
class Command(BaseCommand):
    ...
    def scrape_dwp(self):
        ...
        params = {"pp": "50"}  # 50 results per page
        ...
        try:
                ...
                for job in job_listings:
                    try:
                        ...
                        for li in details:
                            ...
                            if salary == "Not specified" and (
                                "£" in html or "salary" in text.lower()
                            ):
                            ...
                            if "Location:" in text or text.startswith("Based in"):
                                ...
                                city, found_postcode = (
                                    self.cleaner.extract_location_from_text(
                                        location_text
                                    )
                                )
                                if city:
                                ...
                                city, found_postcode = (
                                    self.cleaner.extract_location_from_text(html)
                                )
                                if city:
                                    location = city
                                    postcode = found_postcode
                        ...
                        if location == "Unknown":
                            employer_location = next(
                                (li for li in details if li.find("strong")), None
                            )
                            if employer_location:
                                employer_name = employer_location.find(
                                    "strong"
                                ).text.strip()
                                # Get all text after the employer name, including HTML
                                location_parts = [
                                    str(elem)
                                    for elem in employer_location.contents
                                ...
                                if salary == "Not specified":
                                    for part in location_parts:
                                        if "£" in part:
                                            extracted_salary = (
                                                self.cleaner.extract_salary(part)
                                            )
                                            if extracted_salary != "Not specified":
                                                salary = extracted_salary
                                                break
                        ...
                        if location == "Unknown":
                            try:
                                ...
                                location_elements = [
                                    ...
                                    job_soup.find(
                                        "li",
                                        string=re.compile(r"Based in:", re.IGNORECASE),
                                    ),
                                    (
                                        job_soup.find(
                                            "div", class_="job-profile__header"
                                        ).find("p")
                                        if job_soup.find(
                                            "div", class_="job-profile__header"
                                        )
                                ...
                                if location == "Unknown":
                                    description_div = job_soup.find(
                                        "div", {"id": "job-profile"}
                                    ) or job_soup.find("div", class_="govuk-body")
                                    if description_div:
                                        ...
                                        for pattern in location_patterns:
                                            if match := re.search(
                                                pattern, desc_text, re.IGNORECASE
                                            ):
                                ...
                                self.stdout.write(
                                    self.style.WARNING(
                                        f"Could not fetch job details for location: {str(e)}"
                                    )
                                )
                        ...
                        if location == "Unknown":
                            ...
                            for pattern in title_location_patterns:
                                ...
                                if match:
                                    text = match.group(1).strip()
                                    city, found_postcode = (
                                        self.cleaner.extract_location_from_text(text)
                                    )
                                    if city:
                                        location = city
                                        postcode = found_postcode
                        ...
                        try:
                            ...
                            self.stdout.write(
                                self.style.WARNING(
                                    f"Could not fetch job details: {str(e)}"
                                )
                            )
                            job_details = {}
                            description = ""
                        ...
                        if not job_mode:
                            job_mode = self.cleaner.determine_job_mode(
                                title, description
                            )
                        ...
                        job_data = {
                            ...
                            "mode": job_mode
                            or "full_time",  # Default to full_time if not specified
                            ...
                            "experience_level": self.cleaner.determine_experience_level(
                                title, job_details.get("description", "")
                            ),
                            "skills_required": job_details.get(
                                "skills_required", ""
                        ...
                        if existing_job:
                            ...
                            self.stdout.write(
                                f"⏭️  Skipped duplicate: {job_data['title'][:60]}"
                            )
                        else:
                            job_obj = Opportunity.objects.create(**job_data)
                            created = True
            ...
            self.stdout.write(
                self.style.ERROR(f"Error scraping DWP Find a Job: {str(e)}")
            )
