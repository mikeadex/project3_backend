...
class Command(BaseCommand):
    ...
    def fetch_reed_jobs(self, location="", distance=10, keywords=None):
        ...
        # Build parameters
        params = {"resultsToTake": 100}  # Maximum allowed by API

        # Only add location parameters if a specific location is requested
        if location:
            params.update({"locationName": location, "distanceFromLocation": distance})
        ...
        try:
            ...
            if response.status_code == 500:
                self.stdout.write(
                    self.style.WARNING(
                        "Reed API returned 500 error. Trying with smaller result set..."
                    )
                )
            ...
            self.stdout.write(
                f"Found {total_results} total jobs, processing {len(results)} results"
            )
            ...
            for job in results:
                try:
                    ...
                    description = self.cleaner.clean_text(
                        job.get("jobDescription", "").strip()
                    )
                    ...
                    town = job.get("town", "")
                    job_location = (
                        self.cleaner.extract_location(location_name)
                        if location_name
                        else self.cleaner.extract_location(town)
                    )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error fetching jobs from Reed: {str(e)}")
            )
