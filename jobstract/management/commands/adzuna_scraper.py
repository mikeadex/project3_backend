...
class Command(BaseCommand):
    ...
    def fetch_adzuna_jobs(self, location='UK', keywords='', max_days_old=7, results_per_page=50, force_update=False):
        ...
        try:
            ...
            self.stdout.write(f'📋 Found {total_count} total jobs, processing {len(results)} results')
            ...
            for job in results:
                try:
                    ...
                    employer, _ = Employer.objects.get_or_create(
                        ...
                        defaults={
                            'employer_website': job.get('redirect_url', '')
                        }
                    )
                    ...
                    if salary_min and salary_max:
                        salary_range = f'£{salary_min:,.0f} to £{salary_max:,.0f} per year'
                    elif salary_min:
                    ...
                    if created_str:
                        try:
                            ...
                            date_posted = datetime.fromisoformat(created_str.replace('Z', '+00:00'))
                        except (ValueError, AttributeError):
                            self.stdout.write(self.style.WARNING(f'⚠️  Could not parse date: {created_str}'))
                    ...
                    if not description:
                        self.stdout.write(self.style.WARNING(f'⚠️  Skipping job with no description: {job.get("title", "Unknown")}'))
                        skipped_count += 1
                        continue
                    ...
                    if not job_title:
                        self.stdout.write(self.style.WARNING('⚠️  Skipping job with no title'))
                        skipped_count += 1
                        continue
                    ...
                    experience_level = self.cleaner.determine_experience_level(job_title, description)
                    ...
                    if not application_url:
                        self.stdout.write(self.style.WARNING(f'⚠️  Skipping job with no application URL: {job_title}'))
                        skipped_count += 1
                        continue
                    ...
                    if created:
                        ...
                        if force_update:
                            ...
                            skipped_count += 1
                            self.stdout.write(
                                f'⏭️  Skipped duplicate: {job_title[:60]}'
                            )
            ...
            self.stdout.write(self.style.ERROR(f'❌ Error fetching jobs from Adzuna: {str(e)}'))
