...
class LinkedInParserService:
    ...
    def get_profile_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not fetch additional profile data: {str(e)}")
                profile_data = {}
            ...
            if (
                profile_data
                and "firstName" in profile_data
                and "lastName" in profile_data
            ):
    ...
    def parse_and_save_profile(self, linkedin_profile: LinkedInProfile) -> None:
        ...
        try:
            ...
            try:
                ...
                linkedin_profile.error_message = (
            ...
            try:
                ...
                logger.error(f"Error creating CV Writer: {str(e)}")
                linkedin_profile.sync_status = "failed"
            ...
            try:
                ...
                self._parse_education(
                    linkedin_profile,
                    profile_data.get("educations", {}).get("elements", []),
                )
                ...
                self._parse_experience(
                    linkedin_profile,
                    profile_data.get("positions", {}).get("elements", []),
                )
                self._transfer_experience_to_cv(linkedin_profile, cv_writer)

                # Parse and save skills
                self._parse_skills(
                    linkedin_profile, profile_data.get("skills", {}).get("elements", [])
                )
                ...
                self._parse_certifications(
                    linkedin_profile,
                    profile_data.get("certifications", {}).get("elements", []),
                )
                ...
                self._parse_languages(
                    linkedin_profile,
                    profile_data.get("languages", {}).get("elements", []),
                )

            except Exception as e:
                logger.error(f"Error parsing profile sections: {str(e)}")
                linkedin_profile.sync_status = "failed"
                linkedin_profile.error_message = (
                    f"Failed to parse profile sections: {str(e)}"
                )
                linkedin_profile.save()
                raise
    ...
    def create_cv_writer(
        self, linkedin_profile: LinkedInProfile, profile_data: Dict[str, Any]
    ) -> CvWriter:
    ...
    def _transfer_education_to_cv(
        self, linkedin_profile: LinkedInProfile, cv_writer: CvWriter
    ) -> None:
    ...
    def _transfer_experience_to_cv(
        self, linkedin_profile: LinkedInProfile, cv_writer: CvWriter
    ) -> None:
    ...
    def _transfer_certifications_to_cv(
        self, linkedin_profile: LinkedInProfile, cv_writer: CvWriter
    ) -> None:
    ...
    def _parse_experience(
        self, profile: LinkedInProfile, experience_data: list
    ) -> None:
    ...
    def _parse_certifications(
        self, profile: LinkedInProfile, certifications_data: list
    ) -> None:
    ...
    def _parse_languages(self, profile: LinkedInProfile, languages_data: list) -> None:
        ...
        for lang in languages_data:
            LinkedInLanguage.objects.create(
                profile=profile,
                name=lang.get("name", ""),
                proficiency=self._map_proficiency(
                    lang.get("proficiency", {}).get("level", "")
                ),
    ...
    def get_profile_data_v2(self, access_token):
        ...
        email = (
            email_data.get("elements", [{}])[0].get("handle~", {}).get("emailAddress")
        )
        ...
        try:
            ...
            if "profilePicture" in data:
                ...
                if elements:
                    ...
                    return max(elements, key=lambda x: x["data"]["width"])[
                        "identifiers"
                    ][0]["identifier"]
            return None
        except Exception as e:
    ...
    def update_or_create_profile(
        self, user, access_token, refresh_token=None, expires_at=None
    ):
