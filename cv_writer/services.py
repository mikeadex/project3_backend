...
try:
    ...
    logging.getLogger("cv_writer").warning(
        "llama_cpp not available, local Llama model will not be used"
    )
...
class MistralAPIService:
    ...
    def improve_text(self, text, timeout=30):
        ...
        return self.generate_with_system_prompt(
            "You are a helpful assistant that improves text.", text, timeout
        )
    ...
    def generate_with_system_prompt(self, system_prompt, user_prompt, timeout=30):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Groq API error: {response.status_code}, {response.text[:100]}"
                )
...
class LocalLlamaAPIService:
    ...
    def __init__(self):
        ...
        self.model_path = getattr(settings, "LLAMA_MODEL_PATH", None) or os.environ.get(
            "LLAMA_MODEL_PATH"
        )
    ...
    async def generate_with_system_prompt(self, system_prompt, user_prompt, timeout=60):
        ...
        try:
            ...
            if (
                "CV" in system_prompt
                and "segment" in system_prompt.lower()
                and "section" in system_prompt.lower()
            ):
...
<</SYS>>"""
            ...
            )  # Leave margin for system prompt and response

            if input_too_large:
                logger.warning(
                    f"Input might exceed context window ({estimated_tokens} estimated tokens)"
                )
                ...
                if is_cv_segmentation and len(user_prompt) > 5000:
                    ...
                    for section_name, pattern in section_patterns.items():
                        ...
                        if matches:
                            for match in matches[
                                :1
                            ]:  # Take only the first match for each section
                    ...
                    logger.info(
                        f"Compressed CV from {estimated_tokens} tokens to ~{len(user_prompt) // 4} tokens"
                    )
...
{user_prompt} [/INST]"""
            ...
            if is_cv_segmentation and generated_text:
                ...
                if "==========" not in generated_text:
                    ...
                    implied_sections = re.findall(
                        r"\n([A-Z_]{5,})\s*\n", generated_text
                    )
                    for section in implied_sections:
                        section_clean = section.strip()
                        if any(
                            key in section_clean
                            for key in [
                                "PERSONAL",
                                "SUMMARY",
                                "EDUCATION",
            ...
            if "context window" in str(ve):
                ...
                if "segment" in system_prompt.lower():
                    logger.warning(
                        "Context window exceeded, falling back to basic CV structure"
                    )
...
Return only the improved version without any additional explanations."""
    ...
    async def segment_cv(self, text, timeout=60):
        ...
        if (
            use_local_llama
            and hasattr(self, "local_llama_service")
            and self.local_llama_service
        ):
            try:
...
{text}"""
                ...
                if estimated_tokens > 1800:
                    ...
                    logger.info(
                        f"Original CV text is too long ({estimated_tokens} tokens). Optimizing..."
                    )
                    ...
                    experience_match = re.search(
                        r"(?i)(EXPERIENCE|EMPLOYMENT|WORK HISTORY|PROFESSIONAL BACKGROUND)",
                        text,
                    )
                    experience_part = ""
                    if experience_match:
                    ...
                    education_match = re.search(
                        r"(?i)(EDUCATION|ACADEMIC|QUALIFICATIONS|DEGREE)", text
                    )
                    education_part = ""
                    if education_match:
                    ...
                    skills_match = re.search(
                        r"(?i)(SKILLS|COMPETENCIES|EXPERTISE|PROFICIENCIES)", text
                    )
                    skills_part = ""
                    if skills_match:
                    ...
                    logger.info(
                        f"Compressed CV from {len(text)} to {len(optimized_text)} characters"
                    )
                    user_prompt = f"""Extract and organize the following CV into the required sections using the EXACT TEXT from the document.
...
{optimized_text}"""
                ...
                segmented_text = (
                    await self.local_llama_service.generate_with_system_prompt(
                ...
                if segmented_text and "==========" in segmented_text:
                    ...
                    placeholder_count = sum(
                        1 for p in placeholders if p in segmented_text
                    )

                    # If too many placeholders, it's probably not extracting actual content
                    if placeholder_count > 3:
                        logger.warning(
                            "Local Llama segmentation returned too many placeholders"
                        )
...
class LlamaAPIService:
    ...
    def __init__(self):
        ...
        logger.debug(
            f"LLaMA API key from settings: {self.api_key[:10]}..."
            if self.api_key
            else "None"
        )
    ...
    def generate_completion_sync(self, prompt, max_tokens=2000, temperature=0.7):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"LLaMA sync API error: {response.status_code} - {response.text}"
                )
...
class CVImprovementService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized CV Improvement Service with services: {', '.join(self._initialized_services)}"
        )
    ...
    async def enhance_rewrite(self, initial_rewrite, user=None):
        ...
        try:
            ...
            if (
                "professional_summary" not in rewritten_cv
                or not rewritten_cv.get("professional_summary")
                or (
                    isinstance(rewritten_cv.get("professional_summary"), str)
                    and not rewritten_cv["professional_summary"].strip()
                )
            ):

                logger.info(
                    "No professional summary found, generating one based on experience"
                )
                ...
                try:
                    ...
                    if inspect.iscoroutinefunction(self.generate_response):
                        ...
                        generated_summary = self.generate_response(
                ...
                if isinstance(section_content, str):
                    ...
                    try:
                        ...
                        if inspect.iscoroutinefunction(self.generate_response):
                            ...
                            # If it's a regular function, just call it normally
                            improved_content = self.generate_response(
                                system_prompt, user_prompt
                            )

                        # Fallback to original if enhancement failed
                        enhanced_cv[section_name] = (
                            improved_content if improved_content else section_content
                        )
                    except Exception as e:
                        ...
                        if section_name in ["experience", "work_experience", "jobs"]:
                            ...
                            if "description" in item and item["description"]:
                                ...
                                try:
                                    ...
                                    if inspect.iscoroutinefunction(
                                        ...
                                        # If it's a regular function, just call it normally
                                        improved_description = self.generate_response(
                                            system_prompt, user_prompt
                                        )
                                    ...
                                    logger.error(
                                        f"Error enhancing job description: {str(e)}"
                                    )
                            ...
                            if "description" in item and item["description"]:
                                ...
                                try:
                                    ...
                                    logger.error(
                                        f"Error enhancing education description: {str(e)}"
                                    )
            ...
            return {
                ...
                "rewritten_cv": initial_rewrite.get(
                    "rewritten_cv", initial_rewrite.get("improved_sections", {})
                ),
                "new_cv_id": initial_rewrite.get(
                    "new_cv_id"
                ),  # Pass through the CV ID even if enhancement fails
            }
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if hasattr(service, "generate_with_system_prompt"):
                        result = await service.generate_with_system_prompt(
                            system_prompt, user_prompt
                        )
                    else:
                    ...
                    logger.error(
                        f"{service.__class__.__name__} attempt {attempt+1} failed: {str(e)}"
                    )
    ...
    async def _improve_section(self, section: str, content: Dict) -> Dict:
        ...
        try:
            ...
            formatted_prompt = prompt_data["template"].format(
                content=str(content), industry=self._detect_industry(content)
            )

            # Fallback logic
            if hasattr(self, "use_mistral") and self.use_mistral:
                mistral_result = await self.primary_service.improve_text(
                    formatted_prompt
                )
                if mistral_result:
            ...
            if not os.environ.get("DJANGO_SETTINGS_MODULE", "").endswith(
                "production"
            ) and hasattr(self, "llm_service"):
                return {
                    "original": content,
                    "improved": self.llm_service.improve_section(section, content),
                }
...
def save_rewritten_cv_to_database(rewritten_cv_data, user, cv_writer_instance=None):
    ...
    try:
        ...
        logger.info(
            f"Rewritten CV data structure: {json.dumps(data_summary, indent=2)}"
        )
    except Exception as e:
    ...
    if not cv_writer_instance:
        ...
        if cv_count > 0:
            ...
            cv_writer_instance = CvWriter.objects.create(user=user, status="completed")
    ...
    try:
        ...
        if "professional_summary" in rewritten_cv_data:
            if isinstance(rewritten_cv_data["professional_summary"], str):
                professional_summary_text = clean_ai_text(
                    rewritten_cv_data["professional_summary"]
                )
            elif (
                isinstance(rewritten_cv_data["professional_summary"], dict)
                and "summary" in rewritten_cv_data["professional_summary"]
            ):
                professional_summary_text = clean_ai_text(
                    rewritten_cv_data["professional_summary"]["summary"]
                )

        # Check for nested professional_summary in summary field
        elif "summary" in rewritten_cv_data and isinstance(
            rewritten_cv_data["summary"], str
        ):
        ...
        if professional_summary_text:
            ...
            existing_summary = ProfessionalSummary.objects.filter(
                user=user, cv=cv_writer_instance
            ).first()
            ...
            if existing_summary:
                ...
                ProfessionalSummary.objects.create(
                    user=user, cv=cv_writer_instance, summary=professional_summary_text
                )
    except Exception as e:
    ...
    try:
        if "experience" in rewritten_cv_data and isinstance(
            rewritten_cv_data["experience"], list
        ):
            logger.info(
                f"Processing {len(rewritten_cv_data['experience'])} experience items"
            )
            ...
            for exp_data in rewritten_cv_data["experience"]:
                ...
                company_name = exp_data.get("company_name", "") or exp_data.get(
                    "company", ""
                )
                ...
                if not company_name or not job_title:
                    logger.warning(
                        f"Skipping experience entry with empty company or job title: {exp_data}"
                    )
                    continue
                ...
                try:
                    ...
                    existing_exp = Experience.objects.filter(
                        user=user, company_name=company_name, job_title=job_title
                    ).first()
                    ...
                    if existing_exp:
                        ...
                        try:
                            ...
                            if "cv" not in str(field_error):
                                ...
                                logger.warning(
                                    f"Could not set CV field on experience: {str(field_error)}"
                                )
                        ...
                        existing_exp.save()
                        logger.info(
                            f"Updated existing experience: {company_name}, {job_title}"
                        )
                    else:
                        ...
                        try:
                            ...
                            if "Cannot resolve keyword 'cv'" in str(field_error):
                                logger.warning(
                                    "CV field not found in Experience model, trying without it"
                                )
                                Experience.objects.create(
                                    user=user,
                                    company_name=company_name,
                                ...
                                raise
                        logger.info(
                            f"Created new experience: {company_name}, {job_title}"
                        )
    ...
    try:
        if "workExperience" in rewritten_cv_data and isinstance(
            rewritten_cv_data["workExperience"], list
        ):
            logger.info(
                f"Processing {len(rewritten_cv_data['workExperience'])} workExperience items"
            )
            ...
            for exp_data in rewritten_cv_data["workExperience"]:
                ...
                if not company_name or not job_title:
                    logger.warning(
                        f"Skipping workExperience entry with empty company or job title: {exp_data}"
                    )
                    continue
                ...
                # Format dates
                start_date = exp_data.get("start_date", "") or exp_data.get(
                    "startDate", ""
                )
                ...
                is_current = (
                    not end_date
                    or (isinstance(end_date, str) and end_date.lower() == "present")
                    or exp_data.get("current", False)
                )
                ...
                try:
                    ...
                    if existing_exp:
                        ...
                        existing_exp.save()
                        logger.info(
                            f"Updated existing experience from workExperience: {company_name}, {job_title}"
                        )
                    else:
                        # Create new experience
                        try:
                            ...
                            if "Cannot resolve keyword 'cv'" in str(field_error):
                                ...
                                raise
                        logger.info(
                            f"Created new experience from workExperience: {company_name}, {job_title}"
                        )
                except Exception as inner_e:
                    logger.error(
                        f"Error processing workExperience entry: {str(inner_e)}"
                    )
    except Exception as e:
        ...
        if "education" in rewritten_cv_data and isinstance(
            ...
            for edu_data in rewritten_cv_data["education"]:
                ...
                if not school_name and "school" in edu_data:
                    ...
                    logger.info(
                        f"Using 'school' field instead of 'school_name': {school_name}"
                    )

                if not school_name and "institution" in edu_data:
                    school_name = edu_data.get("institution", "")
                    logger.info(
                        f"Using 'institution' field instead of 'school_name': {school_name}"
                    )
                ...
                defaults = {"field_of_study": field_of_study}
                ...
                try:
                    ...
                    try:
                        ...
                        if "Cannot resolve keyword 'cv'" in str(field_error):
                            logger.warning(
                                "CV field not found in Education model, trying without it"
                            )
                            education, created = Education.objects.update_or_create(
                                user=user,
                                school_name=school_name,
                    ...
                    if created:
                        logger.info(
                            f"Created new education record: {school_name}, {degree}"
                        )
                    else:
                        logger.info(
                            f"Updated existing education record: {school_name}, {degree}"
                        )
            ...
            if isinstance(skills_data, list):
                for skill_item in skills_data:
                    ...
                    if isinstance(skill_item, dict):
                        skill_name = skill_item.get("name", "") or skill_item.get(
                            "skill_name", ""
                        )
                        skill_level = skill_item.get("level", "") or skill_item.get(
                            "skill_level", ""
                        )
                    ...
                    try:
                        ...
                        existing_skill = Skill.objects.filter(
                            user=user, skill_name=skill_name
                        ).first()
                        ...
                        if existing_skill:
                            ...
                            existing_skill.cv = (
                                cv_writer_instance  # Associate with this CV
                            )
                            existing_skill.save()
                            logger.info(
                                f"Updated existing skill: {skill_name}, {skill_level}"
                            )
                        else:
                            # Create new skill
                            ...
                            logger.info(
                                f"Created new skill: {skill_name}, {skill_level}"
                            )
                    except Exception as inner_e:
                ...
                for skill_text in skill_items:
                    ...
                    try:
                        ...
                        if existing_skill:
                            ...
                            existing_skill.save()
                            logger.info(
                                f"Updated existing skill from text: {skill_name}, {skill_level}"
                            )
                        else:
                            # Create new skill
                            Skill.objects.create(
                            ...
                            logger.info(
                                f"Created new skill from text: {skill_name}, {skill_level}"
                            )
                    except Exception as inner_e:
                        logger.error(
                            f"Error processing skill from text: {str(inner_e)}"
                        )
    except Exception as e:
    ...
    try:
        if "languages" in rewritten_cv_data and isinstance(
            rewritten_cv_data["languages"], list
        ):
            ...
            for lang_data in rewritten_cv_data["languages"]:
                ...
                if isinstance(lang_data, dict):
                    language_name = lang_data.get("language", "") or lang_data.get(
                        "name", ""
                    )
                    proficiency = lang_data.get("proficiency", "") or lang_data.get(
                        "level", ""
                    )
                ...
                try:
                    ...
                    existing_lang = Language.objects.filter(
                        user=user, language=language_name
                    ).first()
                    ...
                    if existing_lang:
                        ...
                        try:
                            ...
                            if "cv" not in str(field_error):
                                # Only log if it's not a missing field error
                                logger.warning(
                                    f"Could not set CV field on language: {str(field_error)}"
                                )
                        existing_lang.save()
                        logger.info(
                            f"Updated existing language: {language_name}, {proficiency}"
                        )
                    else:
                        # Create new language
                        try:
                            ...
                            if "Cannot resolve keyword 'cv'" in str(field_error):
                                logger.warning(
                                    "CV field not found in Language model, trying without it"
                                )
                                Language.objects.create(
                                    user=user,
                                    language=language_name,
                                ...
                                raise
                        logger.info(
                            f"Created new language: {language_name}, {proficiency}"
                        )
                except Exception as inner_e:
        ...
        if "certifications" in rewritten_cv_data and isinstance(
            ...
            for cert_data in rewritten_cv_data["certifications"]:
                ...
                if isinstance(cert_data, dict):
                    ...
                    issuing_org = cert_data.get("issuer", "") or cert_data.get(
                    ...
                    date_obtained = (
                        cert_data.get("date", "")
                        or cert_data.get("date_obtained", "")
                        or cert_data.get("year", "")
                    )
                    if not date_obtained or date_obtained.strip() == "":
                        ...
                        if len(date_obtained) == 4 and date_obtained.isdigit():
                            date_obtained = (
                                f"{date_obtained}-01-01"  # Convert year to full date
                            )

                    certificate_link = cert_data.get("url", "") or cert_data.get(
                        "link", ""
                    )
                ...
                try:
                    ...
                    existing_cert = Certification.objects.filter(
                        user=user, certificate_name=cert_name
                    ).first()
                    ...
                    if existing_cert:
                        ...
                        try:
                            ...
                            if "cv" not in str(field_error):
                                # Only log if it's not a missing field error
                                logger.warning(
                                    f"Could not set CV field on certification: {str(field_error)}"
                                )
                        existing_cert.save()
                        ...
                        try:
                            ...
                            if "Cannot resolve keyword 'cv'" in str(field_error):
                                logger.warning(
                                    "CV field not found in Certification model, trying without it"
                                )
                                Certification.objects.create(
                                    user=user,
                                    certificate_name=cert_name,
    ...
    try:
        if "interests" in rewritten_cv_data and isinstance(
            rewritten_cv_data["interests"], list
        ):
            ...
            for interest_data in rewritten_cv_data["interests"]:
                ...
                if isinstance(interest_data, dict):
                    interest_name = interest_data.get("name", "") or interest_data.get(
                        "interest", ""
                    )
                    ...
                    if existing_interest:
                        ...
                        try:
                            ...
                            if "cv" not in str(field_error):
                                # Only log if it's not a missing field error
                                logger.warning(
                                    f"Could not set CV field on interest: {str(field_error)}"
                                )
                        ...
                        try:
                            ...
                            Interest.objects.create(
                                user=user, cv=cv_writer_instance, name=interest_name
                            )
                        except Exception as field_error:
                            # If cv field isn't available, try without it
                            if "Cannot resolve keyword 'cv'" in str(field_error):
                                logger.warning(
                                    "CV field not found in Interest model, trying without it"
                                )
    ...
    try:
        if "references" in rewritten_cv_data and isinstance(
            rewritten_cv_data["references"], list
        ):
            ...
            for ref_data in rewritten_cv_data["references"]:
                ...
                if isinstance(ref_data, dict):
                    ...
                    ref_title = ref_data.get("title", "") or ref_data.get(
                        "position", ""
                    )
                    ref_company = ref_data.get("company", "") or ref_data.get(
                        "organization", ""
                    )
                ...
                try:
                    ...
                    existing_ref = Reference.objects.filter(
                        user=user, name=ref_name, company=ref_company
                    ).first()
                    ...
                    if existing_ref:
                        ...
                        try:
                            ...
                            if "cv" not in str(field_error):
                                # Only log if it's not a missing field error
                                logger.warning(
                                    f"Could not set CV field on reference: {str(field_error)}"
                                )
                        if ref_title:
                        ...
                        try:
                            ...
                            if "Cannot resolve keyword 'cv'" in str(field_error):
                                logger.warning(
                                    "CV field not found in Reference model, trying without it"
                                )
                                Reference.objects.create(
                                    user=user,
                                    name=ref_name,
