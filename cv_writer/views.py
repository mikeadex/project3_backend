...
def improve_cv(request, cv_id):
    ...
    try:
        ...
        return Response(
            {"error": "CV not found or access denied"}, status=status.HTTP_404_NOT_FOUND
        )
...
def improve_section(request):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error in LLM service for section '{section}': {str(e)}")
            return Response(
        ...
        logger.error(f"Unexpected error in improve_section: {str(e)}")
        return Response(
...
def improve_entire_cv(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": "Failed to initialize AI service", "details": str(e)},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )

        # Improve Professional Summary
        try:
            professional_summary = ProfessionalSummary.objects.filter(
                user=request.user, cv=cv
            ).first()
...
Return ONLY the improved professional summary:"""
        ...
        try:
            experiences = Experience.objects.filter(user=request.user).order_by(
                "-start_date"
            )
...
Return ONLY the improved experience bullet points:"""
        ...
        try:
            ...
            if skills.exists():
                skills_text = ", ".join(
                    [f"{skill.skill_name} ({skill.skill_level})" for skill in skills]
                )
    ...
    try:
        ...
        if section == "professional_summary":
            ...
            professional_summary, created = ProfessionalSummary.objects.get_or_create(
                user=request.user, cv=cv, defaults={"summary": improved_content}
            )
            if not created:
            ...
            try:
                ...
                if exp_index < len(experiences):
                    ...
                    experience.achievements = (
                        improved_content  # Set improved content as achievements
                    )
                    experience.save()
            ...
            try:
                ...
                return Response(
                    {"error": "Failed to update skills", "details": str(e)},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    try:
        ...
        if cv_id:
            try:
                ...
                try:
                    professional_summary = ProfessionalSummary.objects.get(
                        user=request.user, cv=cv
                    )
...
Return ONLY the improved summary text, nothing else:"""
        ...
        improved_summary = async_to_sync(
        ...
        if improved_summary:
            ...
            paragraphs = [
                p.strip() for p in improved_summary.split("\n\n") if p.strip()
            ]
            if paragraphs:
                ...
                for paragraph in paragraphs:
                    ...
                    if (
                        ...
                        and not paragraph.upper().startswith(
            ...
            improved_summary = (
                improved_summary.replace(">", "").replace("*", "").strip()
            )

            # Ensure it's not empty and not just the original
            if (
                len(improved_summary) < 20
                or improved_summary.lower() == summary.lower()
            ):
        ...
        if cv_id:
            try:
                ...
                logger.warning(
                    f"Could not update professional summary: {str(update_error)}"
                )

        return Response(
            {"status": "success", "original": summary, "improved": improved_summary},
            status=status.HTTP_200_OK,
        )
...
def rewrite_cv(request):
    ...
    try:
        ...
        if not cv_data:
            return Response(
                {"error": "CV data is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        def process_rewrite_in_background(session_id, cv_data, user_id):
            try:
                ...
                if "parsed_cv" in cv_data and "parsed_data" in cv_data["parsed_cv"]:
                    ...
                    logger.info(
                        f"Extracted CV data from parsed_cv.parsed_data with keys: {list(processed_cv_data.keys())}"
                    )
                else:
                    processed_cv_data = cv_data
                    logger.info(
                        f"Using original cv_data structure with keys: {list(cv_data.keys())}"
                    )
                ...
                try:
                    ...
                    logger.info(
                        f"PROCESSED CV DATA: {json.dumps(processed_cv_data, indent=2)[:1000]}..."
                    )
                except Exception as e:
                ...
                try:
                    ...
                    logger.info(
                        f"Initial rewrite structure: {list(initial_result.keys())}"
                    )
                    logger.info(
                        f"Initial result status: {initial_result.get('status')}"
                    )

                    # Add a direct enhancement using LLaMA - bypass DeepSeek if it didn't work
                    if not initial_result.get(
                        "rewritten_cv"
                    ) and not initial_result.get("improved_sections"):
                        logger.warning(
                            "DeepSeek rewrite didn't produce usable output, enhancing original CV directly with LLaMA"
                        )
                        ...
                        if (
                            "improved_sections" in initial_result
                            and not "rewritten_cv" in initial_result
                        ):
                            initial_result["rewritten_cv"] = initial_result[
                                "improved_sections"
                            ]
                            logger.info(
                                f"Copied improved_sections to rewritten_cv for compatibility"
                            )
                        elif (
                            not "rewritten_cv" in initial_result
                            and not "improved_sections" in initial_result
                        ):
                            # Create a minimal structure to avoid errors
                            logger.warning(
                                f"Neither rewritten_cv nor improved_sections found in initial_result"
                            )
                            initial_result["rewritten_cv"] = processed_cv_data
                            logger.info(
                                f"Created fallback rewritten_cv from original cv_data"
                            )
                    ...
                    logger.info(
                        "🎯 Skipping legacy enhancement - using 3-Layer Quality Control results"
                    )
                    ...
                    final_result = {
                        ...
                        "new_cv_id": enhanced_result.get("new_cv_id")
                        or initial_result.get("new_cv_id"),
                    ...
                    logger.info(
                        f"🔍 Final result approved: {final_result.get('approved')}"
                    )
                    logger.info(
                        f"🔍 Final result quality_score: {final_result.get('quality_score')}"
                    )
                    logger.info(
                        f"🔍 Final result quality_report: {'present' if final_result.get('quality_report') else 'missing'}"
                    )
                    ...
                    if not rewritten_cv:
                        logger.warning(
                            "No rewritten CV data found in results, using processed original"
                        )
                    ...
                    if (
                        rewritten_cv != processed_cv_data
                    ):  # Only merge if we have improved sections
                        logger.info(
                            "Merging improved sections with original CV data to preserve all fields"
                        )
                        ...
                        if "professional_summary" in rewritten_cv:
                            complete_rewritten_cv["professional_summary"] = (
                                rewritten_cv["professional_summary"]
                            )
                            logger.info(
                        ...
                        if "skills" in rewritten_cv:
                            ...
                            if isinstance(rewritten_cv["skills"], str):
                                ...
                                logger.info(
                                    "Skills improvement available but preserving original structure"
                                )
                            else:
                        ...
                        for section in [
                            ...
                            if section in processed_cv_data:
                                complete_rewritten_cv[section] = processed_cv_data[
                                    section
                                ]
                        ...
                        logger.info(
                            "Successfully merged improved sections with original CV data"
                        )
                    ...
                    try:
                        ...
                        rewritten_cv_json = json.dumps(
                            enhanced_result.get("rewritten_cv", {}), indent=2
                        )
                    ...
                    logger.info(
                        f"🔍 Session result keys: {list(session.result.keys()) if isinstance(session.result, dict) else 'Not a dict'}"
                    )
                    logger.info(
                        f"🔍 Session approved: {session.result.get('approved')}"
                    )
                    logger.info(
                        f"🔍 Session quality_score: {session.result.get('quality_score')}"
                    )
                    logger.info(
                        f"🔍 Session quality_report: {'present' if session.result.get('quality_report') else 'missing'}"
                    )

                    # Store the new CV ID in the session
                    new_cv_id = enhanced_result.get("new_cv_id") or initial_result.get(
                        "new_cv_id"
                    )
                    ...
                    try:
                        ...
                        rewritten_cv = save_rewritten_cv_to_database(
                            final_result["rewritten_cv"], user
                        )
                        ...
                        logger.info(
                            f"Saved rewritten CV data to database tables with CV ID: {rewritten_cv.id}"
                        )
                    except Exception as save_error:
                        logger.error(
                            f"Error saving rewritten CV to database tables: {str(save_error)}"
                        )
                    ...
                    logger.error(
                        f"Error in CV rewrite process: {str(e)}", exc_info=True
                    )
                ...
                logger.error(
                    f"Error in background rewrite thread: {str(e)}", exc_info=True
                )
                try:
                    session = CVRewriteSession.objects.get(id=session_id)
                    session.status = "error"
                    ...
                    logger.error(
                        f"Failed to update session after error: {str(inner_e)}"
                    )
...
def get_cv_rewrite_sessions(request, cv_id):
    ...
    try:
        ...
        associated_parsed_cv_ids = list(
            ParsedCV.objects.filter(user=request.user).values_list("id", flat=True)
        )
        ...
        logger.error(
            f"Error fetching rewrite sessions for CV {cv_id}: {str(e)}", exc_info=True
        )
        return Response(
...
def save_rewritten_cv(request):
    ...
    try:
        ...
        # Log the request
        logger.info(
            f"Save rewritten CV request: session_id={session_id}, personal_info={personal_info}"
        )
        ...
        try:
            ...
            logger.info(
                f"Found rewrite session: {session_id}, status: {rewrite_session.status}, new_cv_id: {rewrite_session.new_cv_id}"
            )
        except CVRewriteSession.DoesNotExist:
            return Response(
                {"error": "Rewrite session not found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if not rewrite_session.new_cv_id:
            ...
            if isinstance(session_result, dict) and "rewritten_cv" in session_result:
                try:
                    ...
                    cv_writer_instance = save_rewritten_cv_to_database(
                        session_result["rewritten_cv"], request.user
                    )
                ...
                logger.error(
                    f"Rewrite session does not have rewritten_cv data in result: {session_result}"
                )
                return Response(
                    {
        ...
        try:
            ...
            if rewrite_result and isinstance(rewrite_result, dict):
                ...
                if rewritten_cv_data and "professional_summary" in rewritten_cv_data:
                    professional_summary_text = rewritten_cv_data[
                ...
                if not professional_summary_text:
                    ...
                    if original_cv_data and "professional_summary" in original_cv_data:
                        professional_summary_text = original_cv_data[
                            "professional_summary"
                        ]
                ...
                if professional_summary_text:
                    ...
                    ProfessionalSummary.objects.update_or_create(
                        user=request.user, cv=new_cv, defaults={"summary": cleaned_text}
                    )
            ...
            logger.error(
                f"Error re-saving professional summary and personal info: {str(summary_error)}"
            )
        ...
        if personal_info:
            ...
            if personal_info.get("phone") or personal_info.get("contact_number"):
                new_cv.contact_number = personal_info.get("phone") or personal_info.get(
                    "contact_number"
                )
        ...
        return Response(
            {"message": "Rewritten CV saved successfully", "cv_id": new_cv.id},
            status=status.HTTP_200_OK,
        )
...
def get_cv_improvements(request, cv_id):
    ...
    try:
        ...
        return Response(
            {"status": "success", "improvements": serializer.data},
            status=status.HTTP_200_OK,
        )
...
def get_cv(request, cv_id):
    ...
    try:
        ...
        experiences = Experience.objects.filter(user=request.user, cv=cv).order_by(
            "-start_date"
        )
        education = Education.objects.filter(user=request.user, cv=cv).order_by(
            "-start_date"
        )

        # Get related data FOR THIS SPECIFIC CV
        skills = Skill.objects.filter(user=request.user, cv=cv).exclude(
            Q(skill_name__isnull=True)
            | Q(skill_name="")
            | Q(skill_level__isnull=True)
            | Q(skill_level="")
        )
        ...
        response_data = {
            "professional_summary": (
                ProfessionalSummarySerializer(professional_summary).data.get("summary")
...
class CVVersionListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        try:
            ...
            queryset = CvWriter.objects.filter(user=self.request.user).order_by(
                "-is_primary", "-created_at"
            )
            logger.info(
                f"Fetching CV versions for user {self.request.user.username}. Count: {queryset.count()}"
            )
            return queryset
        except Exception as e:
    ...
    def perform_create(self, serializer):
        try:
            ...
            logger.info(
                f"Creating CV version for user {self.request.user.username}. Is Primary: {is_primary}"
            )
...
class CVVersionDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        return CvWriter.objects.filter(user=self.request.user).order_by(
            "-is_primary", "-created_at"
        )
...
class SetPrimaryVersionView(APIView):
    ...
    def patch(self, request, pk):
        try:
            ...
            CvWriter.objects.filter(user=request.user, is_primary=True).update(
                is_primary=False
            )
...
class CloneCVVersionView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            serializer = CVVersionSerializer(
                cloned_version, context={"request": request}
            )
...
class EditCVVersionView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"error": "CV version not found"}, status=status.HTTP_404_NOT_FOUND
            )
...
def initiate_rewrite_cv(request):
    ...
    try:
        ...
        rewrite_session = CVRewriteSession.objects.create(
            user=request.user, cv_id=cv_id, status="pending"
        )
...
def _process_rewrite_in_background(session_id, cv_id, user_id):
    ...
    try:
        ...
        try:
                    ...
                    if skills.exists():
                        ...
                        for skill in skills:
                            cv_data["skills"].append(
                                {"name": skill.skill_name, "level": skill.skill_level}
                            )
                else:
            ...
            logger.info(
                f"🚀 Starting CV rewrite with 3-Layer Quality Control for user {user.username}"
            )
            ...
            logger.info(
                f"🔍 Session result keys: {list(rewrite_result.keys()) if isinstance(rewrite_result, dict) else 'Not a dict'}"
            )
            if isinstance(rewrite_result, dict):
                logger.info(f"🔍 Session approved: {rewrite_result.get('approved')}")
                logger.info(
                    f"🔍 Session quality_score: {rewrite_result.get('quality_score')}"
                )
                logger.info(
                    f"🔍 Session quality_report: {'present' if rewrite_result.get('quality_report') else 'missing'}"
                )
        ...
        try:
            session = CVRewriteSession.objects.get(id=session_id)
            session.status = "error"
            session.result = {"status": "error", "message": str(e), "error": str(e)}
            session.save()
        except Exception as inner_e:
...
def compare_rewritten_cv(request, session_id):
    ...
    try:
        ...
        if (
            "professional_summary" in original_cv
            and "professional_summary" in rewritten_cv
        ):
            # If it's a dict with content field
            if (
                isinstance(original_cv.get("professional_summary"), dict)
                and "content" in original_cv["professional_summary"]
            ):
...
class CVTemplateListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        organize_by_category = (
            request.query_params.get("organize_by_category", "false").lower() == "true"
        )
...
class SetCVTemplateView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        selection, created = CVTemplateSelection.objects.get_or_create(
            user=request.user, cv=cv, defaults={"template": template}
        )
...
def clear_all_cv_data(request):
    ...
    try:
        ...
        deleted_counts["education"] = (
            education_deleted[0] if education_deleted[0] else 0
        )

        experience_deleted = Experience.objects.filter(user=user).delete()
        deleted_counts["experience"] = (
            experience_deleted[0] if experience_deleted[0] else 0
        )
        ...
        deleted_counts["certifications"] = (
            certification_deleted[0] if certification_deleted[0] else 0
        )
        ...
        deleted_counts["references"] = (
            reference_deleted[0] if reference_deleted[0] else 0
        )

        social_media_deleted = SocialMedia.objects.filter(user=user).delete()
        deleted_counts["social_media"] = (
            social_media_deleted[0] if social_media_deleted[0] else 0
        )

        # Delete professional summaries
        professional_summary_deleted = ProfessionalSummary.objects.filter(
            user=user
        ).delete()
        deleted_counts["professional_summaries"] = (
            professional_summary_deleted[0] if professional_summary_deleted[0] else 0
        )

        # Delete CV improvements
        cv_improvement_deleted = CVImprovement.objects.filter(cv__user=user).delete()
        deleted_counts["cv_improvements"] = (
            cv_improvement_deleted[0] if cv_improvement_deleted[0] else 0
        )

        # Delete template selections
        template_selection_deleted = CVTemplateSelection.objects.filter(
            user=user
        ).delete()
        deleted_counts["template_selections"] = (
            template_selection_deleted[0] if template_selection_deleted[0] else 0
        )

        total_deleted = sum(deleted_counts.values())

        logger.info(
            f"Successfully cleared CV data for user {user.username}. Deleted counts: {deleted_counts}"
        )
        ...
        logger.error(
            f"Error clearing CV data for user {request.user.username}: {str(e)}"
        )
        return Response(
