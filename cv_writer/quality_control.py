...
class CVQualityStandards:
    ...
    FORBIDDEN_PHRASES = [
        "i am", "i have", "i was", "my role", "my responsibility",
        "position at", "work at", "job at"
    ]
    ...
    ATS_KEYWORDS = {
        ...
        'business': [
            "strategy", "analysis", "management", "leadership", "operations",
            "process improvement", "stakeholder", "revenue"
        ],
        'healthcare': [
            "patient care", "clinical", "medical", "healthcare", "treatment",
            "diagnosis", "compliance", "safety"
        ]
    }
...
class WriterAlgorithm:
    ...
    def _sync_generate(self, prompt: str) -> str:
        ...
        try:
            ...
            if hasattr(self.llm_service, 'generate_completion_sync'):
                return self.llm_service.generate_completion_sync(prompt, max_tokens=1000, temperature=0.7)
                ...
                logger.warning(f"LLM service missing expected methods, using mock response")
                return "Enhanced professional content with industry-specific optimization."
        except Exception as e:
            logger.warning(f"LLM service call failed: {str(e)}, using fallback")
            return "Enhanced professional content with improved formatting and keywords."
        
    def generate_content(self, cv_data: Dict[str, Any], industry: str = "technology") -> LayerResult:
        ...
        try:
            ...
            logger.info(f"🖊️ {self.layer_name}: Completed in {processing_time:.2f}s, Score: {metrics.overall_score}")
            return result
    ...
    def _generate_experience(self, experiences: List[Dict], industry: str) -> List[Dict]:
        ...
        for exp in experiences:
            ...
            description = (exp.get('job_description') or exp.get('description') or 
                          exp.get('job_desc') or exp.get('responsibilities'))
            
            if description:
                # Extract fields from both formats
                job_title = (exp.get('job_title') or exp.get('title') or 
                           exp.get('position') or 'Professional')
                company_name = (exp.get('company_name') or exp.get('company') or 
                              exp.get('employer') or 'Company')
    ...
    def _generate_skills(self, skills, industry: str) -> str:
        ...
        if not skills:
            ...
            skills_text = ", ".join([skill.get('skill_name', '') for skill in skills if skill.get('skill_name')])
        else:
    ...
    def _evaluate_content(self, content: Dict[str, Any], industry: str) -> QualityMetrics:
        ...
        overall_score = (content_score + language_score + structure_score + ats_score) / 4
    ...
    def _score_language_quality(self, content: Dict[str, Any]) -> int:
        ...
        forbidden_count = sum(1 for phrase in CVQualityStandards.FORBIDDEN_PHRASES 
        ...
        action_verb_count = sum(1 for verb in CVQualityStandards.REQUIRED_ACTION_VERBS 
    ...
    def _score_structure(self, content: Dict[str, Any]) -> int:
        ...
        present_sections = sum(1 for section in required_sections if content.get(section))
    ...
    def _score_ats_optimization(self, content: Dict[str, Any], industry: str) -> int:
        ...
        keyword_matches = sum(1 for keyword in industry_keywords if keyword in text_content)
    ...
    def _clean_response(self, response: str) -> str:
        ...
        response = re.sub(r'^(here is|here are|improved|rewritten):?\s*', '', response, flags=re.IGNORECASE)
        response = re.sub(r'^(of course|certainly):?\s*', '', response, flags=re.IGNORECASE)
        ...
        for line in lines:
            ...
            if line:
                ...
                elif not line.startswith('-') and any(line.lower().startswith(verb) for verb in 
                    ['led', 'managed', 'developed', 'implemented', 'achieved', 'increased', 'reduced', 
                     'coordinated', 'supervised', 'directed', 'executed', 'delivered', 'oversaw', 
                     'established', 'created', 'designed', 'improved', 'enhanced', 'optimized']):
                    line = f"- {line}"
...
class ReviewerAlgorithm:
    ...
    def review_content(self, writer_result: LayerResult, industry: str = "technology") -> LayerResult:
        ...
        try:
            ...
            metrics = self._evaluate_improvements(reviewed_content, writer_result.metrics, industry)
            ...
            logger.info(f"🔍 {self.layer_name}: Completed in {processing_time:.2f}s, Score improved to: {metrics.overall_score}")
            return result
    ...
    def _evaluate_improvements(self, content: Dict[str, Any], previous_metrics: QualityMetrics, industry: str) -> QualityMetrics:
    ...
    def _clean_response(self, response: str) -> str:
        ...
        # Remove common LLM response prefixes
        response = re.sub(r'^(here is|here are|enhanced|improved):?\s*', '', response, flags=re.IGNORECASE)
        response = re.sub(r'^(of course|certainly):?\s*', '', response, flags=re.IGNORECASE)
...
class ApproverAlgorithm:
    ...
    def approve_content(self, reviewer_result: LayerResult, industry: str = "technology") -> LayerResult:
        ...
        try:
            ...
            validation_results = self._validate_standards(reviewer_result.content, industry)
            ...
            recommendations = self._generate_final_recommendations(validation_results, final_metrics)
            ...
            logger.info(f"✅ {self.layer_name}: {status} in {processing_time:.2f}s, Final Score: {final_metrics.overall_score}")
    ...
    def _validate_standards(self, content: Dict[str, Any], industry: str) -> Dict[str, bool]:
        """Validate content against quality standards"""
        results = {}
        
        # Content length standards
        results['summary_length'] = self._validate_summary_length(content.get('professional_summary', ''))
        results['experience_count'] = self._validate_experience_count(content.get('experience', []))
        ...
        results['industry_keywords'] = self._validate_industry_keywords(content, industry)
    ...
    def _validate_summary_length(self, summary: str) -> bool:
        ...
        return (self.standards.MIN_SUMMARY_LENGTH <= 
                len(summary) <= 
    ...
    def _validate_action_verbs(self, content: Dict[str, Any]) -> bool:
        ...
        text = self._extract_all_text(content).lower()
        action_verb_count = sum(1 for verb in self.standards.REQUIRED_ACTION_VERBS 
                               if verb in text)
    ...
    def _validate_industry_keywords(self, content: Dict[str, Any], industry: str) -> bool:
    ...
    def _validate_completeness(self, content: Dict[str, Any]) -> bool:
        ...
        experiences = content.get('experience', [])
        detailed_exp = sum(1 for exp in experiences 
    ...
    def _generate_final_metrics(self, content: Dict[str, Any], validation_results: Dict[str, bool], 
                               previous_metrics: QualityMetrics, industry: str) -> QualityMetrics:
        """Generate final quality metrics"""
        
        # Calculate validation score
        validation_score = sum(validation_results.values()) / len(validation_results) * 5
        
        # Combine with previous metrics (weighted average)
        final_content_score = min(5, int((previous_metrics.content_score + validation_score) / 2))
        final_language_score = min(5, previous_metrics.language_score + (1 if validation_results.get('professional_tone') else 0))
        final_structure_score = min(5, previous_metrics.structure_score + (1 if validation_results.get('structure_consistency') else 0))
        final_ats_score = min(5, previous_metrics.ats_score + (1 if validation_results.get('industry_keywords') else 0))
        
        final_overall_score = (final_content_score + final_language_score + final_structure_score + final_ats_score) / 4
        
        # Generate comprehensive feedback
        feedback = []
        for standard, passed in validation_results.items():
            if not passed:
                feedback.append(f"Failed standard: {standard.replace('_', ' ').title()}")
        
        if not feedback:
            feedback.append("All quality standards met - content approved for publication")
    ...
    def _generate_final_recommendations(self, validation_results: Dict[str, bool], 
                                      metrics: QualityMetrics) -> List[str]:
        """Generate final recommendations"""
        recommendations = []
        
        if metrics.passed_standards:
            recommendations.append("✅ Content meets all quality standards and is ready for publication")
            ...
            recommendations.append("⚠️ Content requires additional refinement before publication")
            
            # Specific recommendations based on failed validations
            failed_standards = [standard for standard, passed in validation_results.items() if not passed]
            
            for standard in failed_standards:
                if standard == 'summary_length':
                    recommendations.append("📝 Adjust professional summary length (100-300 characters)")
                elif standard == 'experience_count':
                    recommendations.append("💼 Add more detailed work experience entries")
                elif standard == 'skills_count':
                    recommendations.append("🎯 Expand skills section with more relevant competencies")
                elif standard == 'no_forbidden_phrases':
                    recommendations.append("✍️ Remove first-person language and informal phrases")
                elif standard == 'has_action_verbs':
                    recommendations.append("💪 Include more strong action verbs in descriptions")
                elif standard == 'industry_keywords':
                    recommendations.append("🔍 Add more industry-specific keywords for ATS optimization")
                elif standard == 'structure_consistency':
                    recommendations.append("📋 Ensure all required sections are present and properly formatted")
                elif standard == 'career_consistency':
                    recommendations.append("🚨 CRITICAL: Content changes the professional's career field inappropriately - preserve original industry context")
    ...
    def _validate_career_consistency(self, content: Dict[str, Any]) -> bool:
        ...
        retail_indicators = ['retail', 'store', 'sales', 'customer service', 'merchandising', 'shop']
        tech_indicators = ['digital transformation', 'system performance', 'software', 'technology leader', 'it', 'lifecycle initiatives']
        
        healthcare_indicators = ['patient', 'medical', 'clinical', 'hospital', 'healthcare', 'nursing']
        finance_indicators = ['banking', 'financial', 'investment', 'accounting', 'audit', 'credit']
        ...
        if retail_count >= 3 and tech_count >= 2:
            logger.warning("🚨 Career consistency violation: Retail professional described as technology leader")
            return False
            
        # Similar checks for other inappropriate field changes
        healthcare_count = sum(1 for indicator in healthcare_indicators if indicator in text)
        ...
        if healthcare_count >= 2 and finance_count >= 2:
            logger.warning("🚨 Career consistency violation: Healthcare to finance field change detected")
            return False
...
class ThreeLayerQualityController:
    ...
    def __init__(self, writer_llm_service, reviewer_llm_service=None):
        ...
        review_service = reviewer_llm_service if reviewer_llm_service else writer_llm_service
        self.reviewer = ReviewerAlgorithm(review_service)
        
        self.approver = ApproverAlgorithm()
        
        logger.info(f"🎯 Initialized 3-Layer QC: Writer({type(writer_llm_service).__name__}), Reviewer({type(review_service).__name__})")
    ...
    def _format_experience_for_frontend(self, experiences: List[Dict]) -> str:
        ...
        for exp in experiences:
            ...
            description = (exp.get('job_description') or exp.get('description') or 
                          exp.get('job_desc') or exp.get('responsibilities') or '')
            ...
            if start_date and end_date:
                ...
                # Fallback to check for legacy dates field
                dates = exp.get('dates') or exp.get('date_range') or ''
            
            # Format the section
            header = f"{job_title} at {company_name}"
            if dates:
                section = f"{header}\n{dates}\n{description}"
            else:
                section = f"{header}\n{description}"
    ...
    def process_cv(self, cv_data: Dict[str, Any], user: User, 
        ...
        try:
            ...
            if 'experience' in formatted_content and isinstance(formatted_content['experience'], list):
                formatted_content['experience'] = self._format_experience_for_frontend(formatted_content['experience'])
            ...
            logger.info(f"🎯 3-Layer QC Complete: {status} | Score: {approver_result.metrics.overall_score:.2f} | Time: {total_time:.2f}s")
